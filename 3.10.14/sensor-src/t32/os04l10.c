 ...
 *  0           2560*1440       25        mipi_2lane    linear  10        25
 * @I2C addr:0x3c, 0x3d
 *
 * @FSync:none
 *
...
#define SENSOR_VERSION  "H20260123a"
...
unsigned int os04l10_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
    ...
    while (lut->gain <= os04l10_attr.again.max) {
        if (isp_gain == 0) {
            ...
            *sensor_again = (lut - 1)->value;
            ...
            if ((lut->gain == os04l10_attr.again.max) && (isp_gain >= lut->gain)) {
...
#ifdef SENSOR_WDR_2_FRAME
unsigned int os04l10_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
#ifndef SENSOR_TEST
#ifdef SENSOR_AGAIN_TABLE
    /* Analog gain table */
    struct again_lut *lut = os04l10_again_lut;
    while(lut->gain <= os04l10_attr.max_again_short) {
        if(isp_gain == 0) {
            *sensor_again = 0;
            return 0;
        }
            ...
            *sensor_again = (lut - 1)->value;
            ...
            if((lut->gain == os04l10_attr.max_again_short) && (isp_gain >= lut->gain)) {
                *sensor_again = lut->value;
                return lut->gain;
            }
        }
...
#ifdef SENSOR_HCG
unsigned int os04l10_alloc_hcg(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
unsigned int os04l10_alloc_hcg_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
struct tx_isp_mipi_bus os04l10_mipi_linear = {
...
struct tx_isp_dvp_bus os04l10_dvp = {
...
#ifdef SENSOR_WDR_2_FRAME
    .sensor_ctrl.alloc_again_short = os04l10_alloc_again_short,
#endif /* SENSOR_WDR_2_FRAME */
...
#ifdef SENSOR_I2C_REG_8BIT
int os04l10_read(struct tx_isp_subdev *sd, unsigned char reg,
                 unsigned char *value)
{
...
int os04l10_write(struct tx_isp_subdev *sd, unsigned char reg,
                  unsigned char value)
{
    struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int os04l10_clk_set(struct tx_isp_subdev *sd, struct clk *sclka, int mclk)
{
...
static int os04l10_attr_set(struct tx_isp_subdev *sd, struct tx_isp_sensor_win_setting *wise)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    int ret = ISP_SUCCESS;
...
static int os04l10_setting_select(struct tx_isp_subdev *sd, int deboot)
{
    int ret = ISP_SUCCESS;
    ...
    case 0:
        ...
        os04l10_attr.again.value = 65536;
        os04l10_attr.again.max = 259142;
        os04l10_attr.again.min = 0;
        os04l10_attr.again.apply_delay = 2;

        os04l10_attr.integration_time.value = 0x7b6;
        os04l10_attr.integration_time.max = 1764 - 9;
        os04l10_attr.integration_time.min = 4;
        os04l10_attr.integration_time.apply_delay = 2;

        os04l10_attr.total_width = 816;
        os04l10_attr.total_height = 1764;

        os04l10_attr.expo_fs = TX_SENSOR_EXPO_FRAME_START;

#ifdef SENSOR_HCG
        os04l10_attr.hcg.base_gain = ;
        os04l10_attr.hcg.apply_delay = ;
#endif /* SENSOR_HCG */

#ifdef SENSOR_WDR_2_FRAME
        os04l10_attr.again_short.value = ;
        os04l10_attr.again_short.max = ;
        os04l10_attr.again_short.min = ;
        os04l10_attr.again_short.apply_delay = ;

        os04l10_attr.integration_time_short.value = ;
        os04l10_attr.integration_time_short.max = ;
        os04l10_attr.integration_time_short.min = ;
        os04l10_attr.integration_time_short.apply_delay = ;

        os04l10_attr.wdr_cache = wdr_line * os04l10_attr.total_width;

#ifdef SENSOR_HCG
        os04l10_attr.hcg_short.base_gain = ;
        os04l10_attr.hcg_short.apply_delay = ;
#endif /* SENSOR_HCG */
#endif /* SENSOR_WDR_2_FRAME */

        memcpy((void *)(&(os04l10_attr.mipi)), (void *)(&os04l10_mipi_linear), sizeof(os04l10_attr.mipi));
        break;
    default:
...
static int os04l10_attr_check(struct tx_isp_subdev *sd)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    ...
    os04l10_setting_select(sd, info->default_boot);
    ...
    ret = os04l10_clk_set(sd, sclka, SENSOR_MCLK);
    if (ret) {
                                ...
                                struct tx_isp_chip_ident *chip)
{
    ...
    os04l10_attr_check(sd);
#ifndef ZRT_SENSOR_WITHOUT_INIT
    if (info->rst_gpio != -1) {
        ret = private_gpio_request(info->rst_gpio, "os04l10_reset");
        if (!ret) {
    ...
    if (info->pwdn_gpio != -1) {
        ret = private_gpio_request(info->pwdn_gpio, "os04l10_pwdn");
        if (!ret) {
    ...
    ISP_WARNING("===================================================\n");
    ISP_WARNING("OS04L10 version is %s\n", TVERSION);
...
static int os04l10_set_fps(struct tx_isp_subdev *sd, int fps)
{
    ...
    ret += os04l10_write(sd, 0xfd, 0x01);
    os04l10_write(sd, 0x06, (unsigned char) (vb & 0xff));
    os04l10_write(sd, 0x05, (unsigned char) (vb >> 8));
    ret += os04l10_write(sd, 0x01, 0x01);
...
static int os04l10_set_hvflip(struct tx_isp_subdev *sd, void *arg)
{
...
static int os04l10_pm_s_stream_ctrl(struct tx_isp_subdev *sd, void *arg)
{
	int ret = ISP_SUCCESS;
	IMPISPUserCtl *data = (IMPISPUserCtl *)arg;

	if(data->cmd == TX_SENSOR_PM_RESUME){
		printk("os04l10 TX_SENSOR_PM_RESUME\n");
		...
		printk("os04l10 TX_SENSOR_PM_SUSPEND !!!\n");
	}else if(data->cmd == TX_SENSOR_PM_PREPARE) {
		printk("\n os04l10 TX_SENSOR_PM_PREPARE !!!\n");
		...
		printk("\n os04l10 TX_SENSOR_PM_STREAMOFF !!!\n");
	}else {
...
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
static int os04l10_set_expo_short(struct tx_isp_subdev *sd, int value)
{
    int ret = ISP_SUCCESS;
...
#else
static int os04l10_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
    int ret = ISP_SUCCESS;
...
static int os04l10_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
    int ret = ISP_SUCCESS;
...
static int os04l10_set_wdr_stop(struct tx_isp_subdev *sd, void *arg)
{
    ...
    if (par->wdr_en == 1) {
        ...
        os04l10_setting_select(sd, 1);
        os04l10_attr_set(sd, wsize);
    } else if (par->wdr_en == 0) {
        if (par->boot == -1) {
            info->default_boot = 0;
        }
        os04l10_setting_select(sd, 0);
        os04l10_attr_set(sd, wsize);
    } else {
...
static int os04l10_set_wdr(struct tx_isp_subdev *sd, void *arg)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    struct tx_isp_wdr_par *par = (struct tx_isp_wdr_par *)arg;
    struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int os04l10_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    case TX_ISP_EVENT_SENSOR_HVFLIP:
        if(arg)
            ret = os04l10_set_hvflip(sd, (void *)sensor_val->value);
        break;
    case TX_ISP_EVENT_PM_S_STREAM_CTRL:
        if(arg)
            ret =  os04l10_pm_s_stream_ctrl(sd, arg);
        break;
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
    case TX_ISP_EVENT_SENSOR_EXPO_SHORT:
        if (arg)
            ret = os04l10_set_expo_short(sd, sensor_val->value);
        break;
#else
    case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
        if(arg)
            ret = os04l10_set_integration_time_short(sd, sensor_val->value);
        break;
    case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
        if(arg)
            ret = os04l10_set_analog_gain_short(sd, sensor_val->value);
        break;
#endif /* SENSOR_EXPO */
    case TX_ISP_EVENT_SENSOR_WDR:
        if(arg)
            ret = os04l10_set_wdr(sd, (void *)sensor_val->value);
        break;
    case TX_ISP_EVENT_SENSOR_WDR_STOP:
        if(arg)
            ret = os04l10_set_wdr_stop(sd, (void *)sensor_val->value);
        break;
#endif /* SENSOR_WDR_2_FRAME */
    default:
        break;
...
#ifdef CONFIG_ZERATUL
static int init_first_os04l10(void) {
    return private_i2c_add_driver(&os04l10_driver);
}

static void exit_first_os04l10(void) {
    private_i2c_del_driver(&os04l10_driver);
}
...
struct tx_isp_sensor_fast_attr sensor0 = {
    ...
    .wdr = 0,
#endif /* SENSOR_WDR_2_FRAME */
    .init_sensor = init_first_os04l10,
    .exit_sensor = exit_first_os04l10
};
#endif	/* CONFIG_ZERATUL */

MODULE_DESCRIPTION("A low-level driver for Sony os04l10 sensor");
MODULE_LICENSE("GPL");
