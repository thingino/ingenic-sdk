...
*  0           2880*1620       20        mipi_2lane    linear  10        20        null
*
* @I2C addr:0x30, 0x32
*
* @FSync:none
...
unsigned int sc5336_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	while (lut->gain <= sc5336_attr.again.max) {
		if (isp_gain == 0) {
			...
			*sensor_again = (lut - 1)->value;
			...
			if ((lut->gain == sc5336_attr.again.max) && (isp_gain >= lut->gain)) {
...
#ifdef SENSOR_WDR_2_FRAME
unsigned int sc5336_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
#ifndef SENSOR_TEST
#ifdef SENSOR_AGAIN_TABLE
	/* Analog gain table */
	struct again_lut *lut = sc5336_again_lut;
	while(lut->gain <= sc5336_attr.again_short.max) {
		if(isp_gain == 0) {
			*sensor_again = 0;
			return 0;
		}
			...
			*sensor_again = (lut - 1)->value;
			...
			if((lut->gain == sc5336_attr.again_short.max) && (isp_gain >= lut->gain)) {
				*sensor_again = lut->value;
				return lut->gain;
			}
		}
...
#ifdef SENSOR_HCG
unsigned int sc5336_alloc_hcg(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
unsigned int sc5336_alloc_hcg_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
struct tx_isp_mipi_bus sc5336_mipi_linear = {
...
struct tx_isp_dvp_bus sc5336_dvp = {
...
#ifdef SENSOR_WDR_2_FRAME
	.sensor_ctrl.alloc_again_short = sc5336_alloc_again_short,
#endif /* SENSOR_WDR_2_FRAME */
...
static struct regval_list sc5336_init_regs_2880_1620_20fps_mipi[] = {
	{0x0103,0x01},
	{0x36e9,0x80},
	{0x37f9,0x80},
	{0x301f,0x2a},
...
/*
* the order of the sc5336_win_sizes is [full_resolution, preview_resolution].
*/
static struct tx_isp_sensor_win_setting sc5336_win_sizes[] = {
	{
		...
		.regs           = sc5336_init_regs_2880_1620_20fps_mipi,
	},
};

static struct tx_isp_sensor_win_setting *wsize = &sc5336_win_sizes[0];
...
#ifdef SENSOR_I2C_REG_8BIT
int sc5336_read(struct tx_isp_subdev *sd, unsigned char reg,
				unsigned char *value)
{
...
int sc5336_write(struct tx_isp_subdev *sd, unsigned char reg,
				unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int sc5336_clk_set(struct tx_isp_subdev *sd, struct clk *sclka, int mclk)
{
...
static int sc5336_attr_set(struct tx_isp_subdev *sd, struct tx_isp_sensor_win_setting *wise)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
...
static int sc5336_setting_select(struct tx_isp_subdev *sd, int deboot)
{
	int ret = ISP_SUCCESS;
	...
	case 0:
		...
		sc5336_attr.again.value = 0;
		sc5336_attr.again.max = 327680;
		sc5336_attr.again.min = 0;
		sc5336_attr.again.apply_delay = 2;

		sc5336_attr.integration_time.value = 0x700;
		sc5336_attr.integration_time.max = 2475 - 8;
		sc5336_attr.integration_time.min = 2;
		sc5336_attr.integration_time.apply_delay = 2;

		sc5336_attr.total_width = 1600;
		sc5336_attr.total_height = 2475;

		sc5336_attr.expo_fs = TX_SENSOR_EXPO_FRAME_START;

#ifdef SENSOR_HCG
		sc5336_attr.hcg.base_gain = ;
		sc5336_attr.hcg.apply_delay = ;
#endif /* SENSOR_HCG */

#ifdef SENSOR_WDR_2_FRAME
		sc5336_attr.again_short.value = ;
		sc5336_attr.again_short.max = ;
		sc5336_attr.again_short.min = ;
		sc5336_attr.again_short.apply_delay = ;

		sc5336_attr.integration_time_short.value = ;
		sc5336_attr.integration_time_short.max = ;
		sc5336_attr.integration_time_short.min = ;
		sc5336_attr.integration_time_short.apply_delay = ;

		sc5336_attr.wdr_cache = wdr_line * sc5336_attr.total_width;

#ifdef SENSOR_HCG
		sc5336_attr.hcg_short.base_gain = ;
		sc5336_attr.hcg_short.apply_delay = ;
#endif /* SENSOR_HCG */
#endif /* SENSOR_WDR_2_FRAME */

		memcpy((void *)(&(sc5336_attr.mipi)), (void *)(&sc5336_mipi_linear), sizeof(sc5336_attr.mipi));
		break;
	default:
...
static int sc5336_attr_check(struct tx_isp_subdev *sd)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	...
	sc5336_setting_select(sd, info->default_boot);
	...
	ret = sc5336_clk_set(sd, sclka, SENSOR_MCLK);
	if (ret) {
								...
								struct tx_isp_chip_ident *chip)
{
	...
	sc5336_attr_check(sd);
#ifndef ZRT_SENSOR_WITHOUT_INIT
	if (info->rst_gpio != -1) {
		ret = private_gpio_request(info->rst_gpio, "sc5336_reset");
		if (!ret) {
	...
	if (info->pwdn_gpio != -1) {
		ret = private_gpio_request(info->pwdn_gpio, "sc5336_pwdn");
		if (!ret) {
...
static int sc5336_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			...
			pr_debug("sc5336 stream on\n");
		}

	} else {
		ret = sc5336_write_array(sd, sc5336_stream_off_mipi);
		sensor->video.state = TX_ISP_MODULE_INIT;
		pr_debug("sc5336 stream off\n");
	}
...
static int sc5336_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	ret += sc5336_write(sd, 0x3e02, (unsigned char)((it & 0xf) << 4));
...
static int sc5336_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	...
	ret += sc5336_write(sd, 0x3e00, (unsigned char)((value >> 12) & 0xf));
	ret += sc5336_write(sd, 0x3e01, (unsigned char)((value >> 4) & 0xff));
	ret += sc5336_write(sd, 0x3e02, (unsigned char)((value & 0xf) << 4));
...
static int sc5336_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	ret += sc5336_write(sd, 0x3e09, (unsigned char)((value >> 8) & 0xff));
...
static int sc5336_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	sc5336_write(sd, 0x320f, (unsigned char) (vts & 0xff));
	sc5336_write(sd, 0x320e, (unsigned char) (vts >> 8));
...
static int sc5336_set_hvflip(struct tx_isp_subdev *sd, void *arg)
{
	...
	ret += sc5336_write(sd, 0x3221, val);
...
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
static int sc5336_set_expo_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
#else
static int sc5336_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
static int sc5336_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
static int sc5336_set_wdr_stop(struct tx_isp_subdev *sd, void *arg)
{
	struct tx_isp_sensor *sensor = tx_isp_get_subdev_hostdata(sd);
	struct tx_isp_sensor_register_info *info = &sensor->info;
	...
	if (par->wdr_en == 1) {
		...
		sc5336_setting_select(sd, 1);
		sc5336_attr_set(sd, wsize);
	} else if (par->wdr_en == 0) {
		if (par->boot == -1) {
			info->default_boot = 0;
		}
		sc5336_setting_select(sd, 0);
		sc5336_attr_set(sd, wsize);
	} else {
...
static int sc5336_set_wdr(struct tx_isp_subdev *sd, void *arg)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	struct tx_isp_wdr_par *par = (struct tx_isp_wdr_par *)arg;
	struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int sc5336_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_HVFLIP:
		if(arg)
			ret = sc5336_set_hvflip(sd, (void *)sensor_val->value);
		break;
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
	case TX_ISP_EVENT_SENSOR_EXPO_SHORT:
		if (arg)
			ret = sc5336_set_expo_short(sd, sensor_val->value);
		break;
#else
	case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
		if(arg)
			ret = sc5336_set_integration_time_short(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
		if(arg)
			ret = sc5336_set_analog_gain_short(sd, sensor_val->value);
		break;
#endif /* SENSOR_EXPO */
	case TX_ISP_EVENT_SENSOR_WDR:
		if(arg)
			ret = sc5336_set_wdr(sd, (void *)sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_WDR_STOP:
		if(arg)
			ret = sc5336_set_wdr_stop(sd, (void *)sensor_val->value);
		break;
#endif /* SENSOR_WDR_2_FRAME */
	default:
		break;
...
#ifdef CONFIG_ZERATUL
static int init_first_sc5336(void) {
	return private_i2c_add_driver(&sc5336_driver);
}

static void exit_first_sc5336(void) {
	private_i2c_del_driver(&sc5336_driver);
}
...
struct tx_isp_sensor_fast_attr sensor0 = {
	...
	.wdr = 0,
#endif /* SENSOR_WDR_2_FRAME */
	.init_sensor = init_first_sc5336,
	.exit_sensor = exit_first_sc5336
};
#endif  /* CONFIG_ZERATUL */

MODULE_DESCRIPTION("A low-level driver for Sony sc5336 sensor");
MODULE_LICENSE("GPL");
