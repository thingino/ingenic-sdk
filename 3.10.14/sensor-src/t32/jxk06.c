 ...
 *   0          2560x1440       25          MIPI        Linear   10      30
 * @I2C addr:
 *
 * @FSync:
 *
...
unsigned int jxk06_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	while (lut->gain <= jxk06_attr.again.max) {
		if (isp_gain == 0) {
			...
			*sensor_again = (lut - 1)->value;
			...
			if ((lut->gain == jxk06_attr.again.max) && (isp_gain >= lut->gain)) {
...
#ifdef SENSOR_WDR_2_FRAME
unsigned int jxk06_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
#ifndef SENSOR_TEST
#ifdef SENSOR_AGAIN_TABLE
	/* Analog gain table */
	struct again_lut *lut = jxk06_again_lut;
	while(lut->gain <= jxk06_attr.again_short.max) {
		if(isp_gain == 0) {
			*sensor_again = 0;
			return 0;
		}
			...
			*sensor_again = (lut - 1)->value;
			...
			if((lut->gain == jxk06_attr.again_short.max) && (isp_gain >= lut->gain)) {
				*sensor_again = lut->value;
				return lut->gain;
			}
		}
...
#ifdef SENSOR_HCG
unsigned int jxk06_alloc_hcg(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
unsigned int jxk06_alloc_hcg_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
struct tx_isp_mipi_bus jxk06_mipi_linear = {
...
struct tx_isp_dvp_bus jxk06_dvp = {
...
#ifdef SENSOR_WDR_2_FRAME
	.sensor_ctrl.alloc_again_short = jxk06_alloc_again_short,
#endif /* SENSOR_WDR_2_FRAME */
...
static struct regval_list jxk06_init_regs_2560_1440_25fps_mipi[] = {
	{0x12,0x70},
	{0x48,0x86},
	{0x48,0x06},
	{0x0E,0x11},
...
static struct tx_isp_sensor_win_setting jxk06_win_sizes[] = {
	/* 2560*1440 [0] */
	{
		...
		.regs           = jxk06_init_regs_2560_1440_25fps_mipi,
	},
};

static struct tx_isp_sensor_win_setting *wsize = &jxk06_win_sizes[0];
...
int jxk06_read(struct tx_isp_subdev *sd, uint16_t reg,
		  unsigned char *value) {
	int ret;
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int jxk06_write(struct tx_isp_subdev *sd, uint16_t reg,
		   unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int jxk06_clk_set(struct tx_isp_subdev *sd, struct clk *sclka, int mclk)
{
...
static int jxk06_attr_set(struct tx_isp_subdev *sd, struct tx_isp_sensor_win_setting *wise)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
...
static int jxk06_setting_select(struct tx_isp_subdev *sd, int deboot)
{
	int ret = ISP_SUCCESS;
	...
	case 0:
		...
		jxk06_attr.again.value = 0x64;
		jxk06_attr.again.max = 259142;
		jxk06_attr.again.min = 0;
		jxk06_attr.again.apply_delay = 2;

		jxk06_attr.integration_time.value = 1500 - 4 ;
		jxk06_attr.integration_time.max = 1500 - 4;
		jxk06_attr.integration_time.min = 1;
		jxk06_attr.integration_time.apply_delay = 2;
...
#ifdef SENSOR_HCG
		jxk06_attr.hcg.base_gain = ;
		jxk06_attr.hcg.apply_delay = ;
#endif /* SENSOR_HCG */

#ifdef SENSOR_WDR_2_FRAME
		jxk06_attr.again_short.value = ;
		jxk06_attr.again_short.max = ;
		jxk06_attr.again_short.min = ;
		jxk06_attr.again_short.apply_delay = ;

		jxk06_attr.integration_time_short.value = ;
		jxk06_attr.integration_time_short.max = ;
		jxk06_attr.integration_time_short.min = ;
		jxk06_attr.integration_time_short.apply_delay = ;

		jxk06_attr.wdr_cache = wdr_line * jxk06_attr.total_width;

#ifdef SENSOR_HCG
		jxk06_attr.hcg_short.base_gain = ;
		jxk06_attr.hcg_short.apply_delay = ;
#endif /* SENSOR_HCG */
#endif /* SENSOR_WDR_2_FRAME */

		memcpy((void *)(&(jxk06_attr.mipi)), (void *)(&jxk06_mipi_linear), sizeof(jxk06_attr.mipi));
		break;
	default:
...
static int jxk06_attr_check(struct tx_isp_subdev *sd)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	...
	jxk06_setting_select(sd, info->default_boot);
	...
	ret = jxk06_clk_set(sd, sclka, SENSOR_MCLK);
	if (ret) {
				 ...
				 struct tx_isp_chip_ident *chip)
{
	...
	ISP_WARNING("===================================================\n");
	ISP_WARNING("jxk06 version is %s\n", TVERSION);
...
static int jxk06_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			...
			pr_debug("jxk06 stream on\n");
		}

	} else {
		ret = jxk06_write_array(sd, jxk06_stream_off_mipi);
		sensor->video.state = TX_ISP_MODULE_INIT;
		pr_debug("jxk06 stream off\n");
	}
...
static int jxk06_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	ret = jxk06_write(sd,  0x01, (unsigned char)(it & 0xff));
	ret += jxk06_write(sd, 0x02, (unsigned char)((it >> 8) & 0xff));
...
static int jxk06_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	jxk06_write(sd, 0x22, (unsigned char) (vts & 0xff));
	jxk06_write(sd, 0x23, (unsigned char) (vts >> 8));
...
static int jxk06_set_hvflip(struct tx_isp_subdev *sd, void *arg)
{
...
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
static int jxk06_set_expo_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
#else
static int jxk06_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
static int jxk06_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
static int jxk06_set_wdr_stop(struct tx_isp_subdev *sd, void *arg)
{
	struct tx_isp_sensor *sensor = tx_isp_get_subdev_hostdata(sd);
	struct tx_isp_sensor_register_info *info = &sensor->info;
	...
	if (par->wdr_en == 1) {
		...
		jxk06_setting_select(sd, 1);
		jxk06_attr_set(sd, wsize);
	} else if (par->wdr_en == 0) {
		if (par->boot == -1) {
			info->default_boot = 0;
		}
		jxk06_setting_select(sd, 0);
		jxk06_attr_set(sd, wsize);
	} else {
...
static int jxk06_set_wdr(struct tx_isp_subdev *sd, void *arg)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	struct tx_isp_wdr_par *par = (struct tx_isp_wdr_par *)arg;
	struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int jxk06_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_HVFLIP:
		if(arg)
			ret = jxk06_set_hvflip(sd, (void *)sensor_val->value);
		break;
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
	case TX_ISP_EVENT_SENSOR_EXPO_SHORT:
		if (arg)
			ret = jxk06_set_expo_short(sd, sensor_val->value);
		break;
#else
	case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
		if(arg)
			ret = jxk06_set_integration_time_short(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
		if(arg)
			ret = jxk06_set_analog_gain_short(sd, sensor_val->value);
		break;
#endif /* SENSOR_EXPO */
	case TX_ISP_EVENT_SENSOR_WDR:
		if(arg)
			ret = jxk06_set_wdr(sd, (void *)sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_WDR_STOP:
		if(arg)
			ret = jxk06_set_wdr_stop(sd, (void *)sensor_val->value);
		break;
#endif /* SENSOR_WDR_2_FRAME */
	default:
		break;
...
static int jxk06_probe(struct i2c_client *client,
...
#ifdef CONFIG_ZERATUL
static int init_first_jxk06(void) {
	return private_i2c_add_driver(&jxk06_driver);
}

static void exit_first_jxk06(void) {
	private_i2c_del_driver(&jxk06_driver);
}
...
struct tx_isp_sensor_fast_attr sensor0 = {
	...
	.wdr = 0,
#endif /* SENSOR_WDR_2_FRAME */
	.init_sensor = init_first_jxk06,
	.exit_sensor = exit_first_jxk06
};
#endif  /* CONFIG_ZERATUL */

MODULE_DESCRIPTION("A low-level driver for jxk06 sensor");
MODULE_LICENSE("GPL");
