 ...
 *  0           2304*1290       30        mipi_2lane    linear  10        30       interior
 *
 * @I2C addr:0x30, 0x32
 *
 * @FSync:none
...
unsigned int sc3336p_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
    ...
    while (lut->gain <= sc3336p_attr.again.max) {
        if (isp_gain == 0) {
            ...
            *sensor_again = (lut - 1)->value;
            ...
            if ((lut->gain == sc3336p_attr.again.max) && (isp_gain >= lut->gain)) {
...
#ifdef SENSOR_WDR_2_FRAME
unsigned int sc3336p_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
#ifndef SENSOR_TEST
#ifdef SENSOR_AGAIN_TABLE
    /* Analog gain table */
    struct again_lut *lut = sc3336p_again_lut;
    while(lut->gain <= sc3336p_attr.again_short.max) {
        if(isp_gain == 0) {
            *sensor_again = 0;
            return 0;
        }
            ...
            *sensor_again = (lut - 1)->value;
            ...
            if((lut->gain == sc3336p_attr.again_short.max) && (isp_gain >= lut->gain)) {
                *sensor_again = lut->value;
                return lut->gain;
            }
        }
...
#ifdef SENSOR_HCG
unsigned int sc3336p_alloc_hcg(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
unsigned int sc3336p_alloc_hcg_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
struct tx_isp_mipi_bus sc3336p_mipi_linear = {
...
struct tx_isp_dvp_bus sc3336p_dvp = {
...
#ifdef SENSOR_WDR_2_FRAME
    .sensor_ctrl.alloc_again_short = sc3336p_alloc_again_short,
#endif /* SENSOR_WDR_2_FRAME */
...
/*
 * the order of the sc3336p_win_sizes is [full_resolution, preview_resolution].
 */
...
static struct tx_isp_sensor_win_setting *wsize = &sc3336p_win_sizes[0];

/*
 * the part of driver was fixed.
 */

static struct regval_list sc3336p_stream_on_mipi[] = {
    {0x0100, 0x01},
    { SENSOR_REG_END, 0x00 }, /* END MARKER */
};

static struct regval_list sc3336p_stream_off_mipi[] = {
    {0x0100, 0x00},
    { SENSOR_REG_END, 0x00 }, /* END MARKER */
};

#ifdef SENSOR_I2C_REG_8BIT
int sc3336p_read(struct tx_isp_subdev *sd, unsigned char reg,
                  unsigned char *value)
{
...
int sc3336p_write(struct tx_isp_subdev *sd, unsigned char reg,
                   unsigned char value)
{
    struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int sc3336p_read(struct tx_isp_subdev *sd, uint16_t reg,
                  unsigned char *value) {
    int ret;
    struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int sc3336p_write(struct tx_isp_subdev *sd, uint16_t reg,
                   unsigned char value)
{
    struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int sc3336p_clk_set(struct tx_isp_subdev *sd, struct clk *sclka, int mclk)
{
...
static int sc3336p_attr_set(struct tx_isp_subdev *sd, struct tx_isp_sensor_win_setting *wise)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    int ret = ISP_SUCCESS;
...
static int sc3336p_setting_select(struct tx_isp_subdev *sd, int deboot)
{
    int ret = ISP_SUCCESS;
    ...
    case 0:
        ...
        sc3336p_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;

        sc3336p_attr.again.value = 65536;
        sc3336p_attr.again.max = 365791;
        sc3336p_attr.again.min = 0;
        sc3336p_attr.again.apply_delay = 2;

        sc3336p_attr.integration_time.value = 0xb60;
        sc3336p_attr.integration_time.max = 1320 - 8;
        sc3336p_attr.integration_time.min = 2;
        sc3336p_attr.integration_time.apply_delay = 2;

        sc3336p_attr.total_width = 1250;
        sc3336p_attr.total_height = 1320;

        sc3336p_attr.expo_fs = TX_SENSOR_EXPO_FRAME_START;

#ifdef SENSOR_HCG
        sc3336p_attr.hcg.base_gain = ;
        sc3336p_attr.hcg.apply_delay = ;
#endif /* SENSOR_HCG */

#ifdef SENSOR_WDR_2_FRAME
        sc3336p_attr.again_short.value = ;
        sc3336p_attr.again_short.max = ;
        sc3336p_attr.again_short.min = ;
        sc3336p_attr.again_short.apply_delay = ;

        sc3336p_attr.integration_time_short.value = ;
        sc3336p_attr.integration_time_short.max = ;
        sc3336p_attr.integration_time_short.min = ;
        sc3336p_attr.integration_time_short.apply_delay = ;

        sc3336p_attr.wdr_cache = wdr_line * sc3336p_attr.total_width;

#ifdef SENSOR_HCG
        sc3336p_attr.hcg_short.base_gain = ;
        sc3336p_attr.hcg_short.apply_delay = ;
#endif /* SENSOR_HCG */
#endif /* SENSOR_WDR_2_FRAME */

        memcpy((void *)(&(sc3336p_attr.mipi)), (void *)(&sc3336p_mipi_linear), sizeof(sc3336p_attr.mipi));
        break;
    default:
...
static int sc3336p_attr_check(struct tx_isp_subdev *sd)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    ...
    sc3336p_setting_select(sd, info->default_boot);

    switch (info->video_interface) {
    case TISP_SENSOR_VI_MIPI_CSI0:
        sc3336p_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
        sc3336p_attr.mipi.index = 0;
        break;
    case TISP_SENSOR_VI_MIPI_CSI1:
        sc3336p_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
        sc3336p_attr.mipi.index = 1;
        break;
    case TISP_SENSOR_VI_DVP:
        sc3336p_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_DVP;
        break;
    default:
    ...
    ret = sc3336p_clk_set(sd, sclka, SENSOR_MCLK);
    if (ret) {
...
static int sc3336p_g_chip_ident(struct tx_isp_subdev *sd,
                                 struct tx_isp_chip_ident *chip)
{
    ...
    sc3336p_attr_check(sd);
#ifndef ZRT_SENSOR_WITHOUT_INIT
    if (info->rst_gpio != -1) {
        ret = private_gpio_request(info->rst_gpio, "sc3336p_reset");
        if (!ret) {
    ...
    if (info->pwdn_gpio != -1) {
        ret = private_gpio_request(info->pwdn_gpio, "sc3336p_pwdn");
        if (!ret) {
...
static int sc3336p_reset(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
    return 0;
}

static int sc3336p_init(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    int ret = ISP_SUCCESS;
...
static int sc3336p_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    int ret = ISP_SUCCESS;
    ...
    if (init->enable) {
        ...
        if (sensor->video.state == TX_ISP_MODULE_INIT) {
            ret = sc3336p_write_array(sd, sc3336p_stream_on_mipi);
            sensor->video.state = TX_ISP_MODULE_RUNNING;
            pr_debug("sc3336p stream on\n");
        }

    } else {
        ret = sc3336p_write_array(sd, sc3336p_stream_off_mipi);
        sensor->video.state = TX_ISP_MODULE_INIT;
        pr_debug("sc3336p stream off\n");
    }
...
static int sc3336p_set_expo(struct tx_isp_subdev *sd, int value)
{
    ...
    ret += sc3336p_write(sd, 0x3e00, (unsigned char)((it >> 12) & 0xf));
    ret += sc3336p_write(sd, 0x3e01, (unsigned char)((it >> 4) & 0xff));
    ret += sc3336p_write(sd, 0x3e02, (unsigned char)((it & 0xf) << 4));
...
static int sc3336p_set_integration_time(struct tx_isp_subdev *sd, int value)
{
    ...
    ret += sc3336p_write(sd, 0x3e00, (unsigned char)((value >> 12) & 0xf));
    ret += sc3336p_write(sd, 0x3e01, (unsigned char)((value >> 4) & 0xff));
    ret += sc3336p_write(sd, 0x3e02, (unsigned char)((value & 0xf) << 4));
...
static int sc3336p_set_fps(struct tx_isp_subdev *sd, int fps)
{
    ...
    val = 0;
    ret += sc3336p_read(sd, 0x320c, &val);
    hts = val << 8;
    val = 0;
    ret += sc3336p_read(sd, 0x320d, &val);
    hts |= val;
    if (0 != ret) {
    ...
    sc3336p_write(sd, 0x320f, (unsigned char) (vts & 0xff));
    sc3336p_write(sd, 0x320e, (unsigned char) (vts >> 8));
...
static int sc3336p_set_hvflip(struct tx_isp_subdev *sd, void *arg)
{
    ...
    ret = sc3336p_read(sd, 0x3221, &val);
    switch(par->hvflip) {
    ...
    ret += sc3336p_write(sd, 0x3221, val);
...
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
static int sc3336p_set_expo_short(struct tx_isp_subdev *sd, int value)
{
    int ret = ISP_SUCCESS;
...
#else
static int sc3336p_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
    int ret = ISP_SUCCESS;
...
static int sc3336p_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
    int ret = ISP_SUCCESS;
...
static int sc3336p_set_wdr_stop(struct tx_isp_subdev *sd, void *arg)
{
    struct tx_isp_sensor *sensor = tx_isp_get_subdev_hostdata(sd);
    struct tx_isp_sensor_register_info *info = &sensor->info;
    ...
    if (par->wdr_en == 1) {
        ...
        sc3336p_setting_select(sd, 1);
        sc3336p_attr_set(sd, wsize);
    } else if (par->wdr_en == 0) {
        if (par->boot == -1) {
            info->default_boot = 0;
        }
        sc3336p_setting_select(sd, 0);
        sc3336p_attr_set(sd, wsize);
    } else {
...
static int sc3336p_set_wdr(struct tx_isp_subdev *sd, void *arg)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    struct tx_isp_wdr_par *par = (struct tx_isp_wdr_par *)arg;
    struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int sc3336p_pm_s_stream_ctrl(struct tx_isp_subdev *sd, void *arg)
{
	int ret = ISP_SUCCESS;
...
static int sc3336p_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    case TX_ISP_EVENT_SENSOR_EXPO:
        if (arg)
            ret = sc3336p_set_expo(sd, sensor_val->value);
        break;
#else
    case TX_ISP_EVENT_SENSOR_INT_TIME:
        if (arg)
            ret = sc3336p_set_integration_time(sd, sensor_val->value);
        break;
    case TX_ISP_EVENT_SENSOR_AGAIN:
        if (arg)
            ret = sc3336p_set_analog_gain(sd, sensor_val->value);
        break;
#endif /* SENSOR_EXPO */
    case TX_ISP_EVENT_SENSOR_DGAIN:
        if (arg)
            ret = sc3336p_set_digital_gain(sd, sensor_val->value);
        break;
    case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
        if (arg)
            ret = sc3336p_get_black_pedestal(sd, sensor_val->value);
        break;
    case TX_ISP_EVENT_SENSOR_RESIZE:
        if (arg)
            ret = sc3336p_set_mode(sd, sensor_val->value);
        break;
    ...
    case TX_ISP_EVENT_SENSOR_FPS:
        if (arg)
            ret = sc3336p_set_fps(sd, sensor_val->value);
        break;
    case TX_ISP_EVENT_SENSOR_HVFLIP:
        if(arg)
            ret = sc3336p_set_hvflip(sd, (void *)sensor_val->value);
        break;
    case TX_ISP_EVENT_PM_S_STREAM_CTRL:
		if(arg)
			ret = sc3336p_pm_s_stream_ctrl(sd, arg);
		break;
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
    case TX_ISP_EVENT_SENSOR_EXPO_SHORT:
        if (arg)
            ret = sc3336p_set_expo_short(sd, sensor_val->value);
        break;
#else
    case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
        if(arg)
            ret = sc3336p_set_integration_time_short(sd, sensor_val->value);
        break;
    case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
        if(arg)
            ret = sc3336p_set_analog_gain_short(sd, sensor_val->value);
        break;
#endif /* SENSOR_EXPO */
    case TX_ISP_EVENT_SENSOR_WDR:
        if(arg)
            ret = sc3336p_set_wdr(sd, (void *)sensor_val->value);
        break;
    case TX_ISP_EVENT_SENSOR_WDR_STOP:
        if(arg)
            ret = sc3336p_set_wdr_stop(sd, (void *)sensor_val->value);
        break;
#endif /* SENSOR_WDR_2_FRAME */
    default:
        break;
...
#ifdef CONFIG_ZERATUL
static int init_first_sc3336p(void) {
    return private_i2c_add_driver(&sc3336p_driver);
}

static void exit_first_sc3336p(void) {
    private_i2c_del_driver(&sc3336p_driver);
}
...
struct tx_isp_sensor_fast_attr sensor0 = {
    ...
    .wdr = 0,
#endif /* SENSOR_WDR_2_FRAME */
    .init_sensor = init_first_sc3336p,
    .exit_sensor = exit_first_sc3336p
};
#endif  /* CONFIG_ZERATUL */

MODULE_DESCRIPTION("A low-level driver for Sony sc3336p sensor");
MODULE_LICENSE("GPL");
