...
unsigned int gc5613_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
    ...
    /* Analog gain table */
    struct again_lut *lut = gc5613_again_lut;
    while(lut->gain <= gc5613_attr.again_short.max) {
        if(isp_gain == 0) {
            *sensor_again = 0;
            return 0;
        }
            ...
            *sensor_again = (lut - 1)->index;
            ...
            if((lut->gain == gc5613_attr.again_short.max) && (isp_gain >= lut->gain)) {
                *sensor_again = lut->index;
                return lut->gain;
            }
        }
...
struct tx_isp_mipi_bus gc5613_mipi_wdr = {
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 1152,
	.lans = 2,
	.image_twidth = 2880,
...
//version 1.5.0
//00_GC5613_MIPI2L_24M_2960x1666_30fps_raw10_linear
//mclk 24Mhz
...
static struct regval_list gc5613_init_regs_2880_1620_20fps_mipi[] = {
//version 1.5.0
//0A_GC5613_MIPI2L_24M_2960x1666_20fps_raw10_HDR
//mclk 24Mhz
...
static struct regval_list gc5613_init_regs_2960_1666_25fps_mipi[] = {
	{0x03fe,0xf0},
	{0x03fe,0x00},
	{0x03fe,0x10},
	{0x03fe,0x00},
...
static struct tx_isp_sensor_win_setting gc5613_win_sizes[] = {
	...
	},
	{
		...
		.mbus_code      = TISP_VI_FMT_SBGGR10_1X10,
		.colorspace     = TISP_COLORSPACE_SRGB,
		.regs           = gc5613_init_regs_2880_1620_20fps_mipi,
	},
	{
		.width          = 2960,
		.height         = 1666,
		...
		.mbus_code      = TISP_VI_FMT_SBGGR10_1X10,
		.colorspace     = TISP_COLORSPACE_SRGB,
		.regs           = gc5613_init_regs_2960_1666_25fps_mipi,
	}
};
...
int gc5613_read(struct tx_isp_subdev *sd, uint16_t reg,
				  unsigned char *value) {
	int ret;
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int gc5613_write(struct tx_isp_subdev *sd, uint16_t reg,
				   unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int gc5613_setting_select(struct tx_isp_subdev *sd, int deboot)
{
	...
	case 0:
		...
		gc5613_attr.integration_time.value = 0xb54;
		gc5613_attr.integration_time.max = 2917 - 8;
		gc5613_attr.integration_time.min = 2;
		gc5613_attr.integration_time.apply_delay = 2;

		gc5613_attr.total_width = 1200;
		gc5613_attr.total_height = 2917;

		gc5613_attr.expo_fs = TX_SENSOR_EXPO_FRAME_START;
	...
	case 1:
		...
		gc5613_attr.data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
		...
		gc5613_attr.integration_time.value = 0xa0;
		gc5613_attr.integration_time.max = 1488; // ( < (1750 - 94) and (93 * 16))
		gc5613_attr.integration_time.min = 2;
		gc5613_attr.integration_time.apply_delay = 2;

		gc5613_attr.total_width = 0x372;
...
#ifdef SENSOR_WDR_2_FRAME
		gc5613_attr.again_short.value = 65536;
		gc5613_attr.again_short.max = 390142;
		gc5613_attr.again_short.min = 2;
		gc5613_attr.again_short.apply_delay = 2;

		gc5613_attr.integration_time_short.value = 0x10;
		gc5613_attr.integration_time_short.max = 93; //1750 - 1620- 16 -20 - 1 ps: 20 is dummy / 16 is Redundant data
		gc5613_attr.integration_time_short.min = 2;
		gc5613_attr.integration_time_short.apply_delay = 2;

		gc5613_attr.wdr_cache = 93 * 1750 * 2; //no use
		...
		memcpy((void *)(&(gc5613_attr.mipi)), (void *)(&gc5613_mipi_wdr), sizeof(gc5613_attr.mipi));
		break;

	case 2:
		wsize = &gc5613_win_sizes[2];
		gc5613_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		...
		gc5613_attr.integration_time.value = 0x6c6;
		gc5613_attr.integration_time.max = 2100 - 8;
		gc5613_attr.integration_time.min = 2;
		gc5613_attr.integration_time.apply_delay = 2;

		gc5613_attr.total_width = 0x372;
		gc5613_attr.total_height = 3500;
...
#endif /* SENSOR_HCG */
		memcpy((void *)(&(gc5613_attr.mipi)), (void *)(&gc5613_mipi_linear), sizeof(gc5613_attr.mipi));
		gc5613_attr.mipi.image_twidth = 2960;
		gc5613_attr.mipi.image_theight = 1666;
		gc5613_attr.mipi.clk = 848;
		break;
	default:
...
static int gc5613_fsync(struct tx_isp_subdev *sd, struct tx_isp_frame_sync *sync)
{
	uint8_t val;
	uint16_t ret_val;

	gc5613_read(sd, 0x0340, &val);
	ret_val = val << 8;
	gc5613_read(sd, 0x0341, &val);
	ret_val |= val;
	ret_val = (ret_val * 8 / 3) + 4;
	gc5613_write(sd, 0x0340, ret_val >> 8);
	gc5613_write(sd, 0x0341, ret_val & 0xff);
	gc5613_write(sd, 0x027f, 0x03);
	gc5613_write(sd, 0x02f7, 0x01);

	/* step 13.9us */
	gc5613_write(sd, 0x02f5, 0x08);
	gc5613_write(sd, 0x02f6, 0xa6);
...
static int gc5613_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	ret += gc5613_write(sd, 0x0614, val_lut[again].reg614);
	ret += gc5613_write(sd, 0x0615, val_lut[again].reg615);
	ret += gc5613_write(sd, 0x0225, val_lut[again].reg225);
	...
	ret += gc5613_write(sd, 0x00b8, val_lut[again].regb8);
	ret += gc5613_write(sd, 0x00b9, val_lut[again].regb9);
...
static int gc5613_set_expo_short(struct tx_isp_subdev *sd, int value)
{
	...
	/*set integration time*/
	ret = gc5613_write(sd, 0x0201, it & 0xff);
	ret += gc5613_write(sd, 0x0200, (it >> 8) & 0xff);
...
static int gc5613_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
	...
	ret = gc5613_write(sd, 0x0201, value & 0xff);
	ret += gc5613_write(sd, 0x0200, (value>>8) & 0xff);
	if (ret < 0) {
		ISP_ERROR("gc5613_write error  %d\n" ,__LINE__ );
		return ret;
	}
