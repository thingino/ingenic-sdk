...
{
	struct ingenic_dmic_dma *dmic_dma = platform_get_drvdata(pdev);
	if (!dmic_dma)
        return 0;
	if(dmic_dma->desc_pool) {
        dma_pool_destroy(dmic_dma->desc_pool);
        dmic_dma->desc_pool = NULL;
    }
	if(dmic_dma->dma_base){
		devm_iounmap(&pdev->dev, dmic_dma->dma_base);
		dmic_dma->dma_base=NULL;
	}
	if(dmic_dma->fifo_base){
		devm_iounmap(&pdev->dev, dmic_dma->fifo_base);
		dmic_dma->fifo_base=NULL;
	}
