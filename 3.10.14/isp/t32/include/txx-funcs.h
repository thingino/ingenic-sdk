...
extern void *test_malloc_addr;
#define private_vmalloc(size) \
	( \
	  test_malloc_addr = vmalloc(size),\
	  ISP_WARNING("[%s %d] vmalloc %p %d\n", __func__, __LINE__, test_malloc_addr, size), \
	  test_malloc_addr \
	  )
#define private_vfree(addr) \
{ \
	vfree(addr); \
	ISP_WARNING("[%s %d] vfree %p\n", __func__, __LINE__, addr); \
}
#define private_kmalloc(s, gfp) \
	( \
	  test_malloc_addr = kmalloc(s, gfp), \
	  ISP_WARNING("[%s %d] kmalloc %p %d\n", __func__, __LINE__, test_malloc_addr, s), \
	  test_malloc_addr \
	  )
#define private_kfree(addr) \
	( \
	  kfree(addr), \
	  ISP_WARNING("[%s %d] kfree %p\n", __func__, __LINE__, addr) \
	  )
#endif /* MALLOC_CHECK */
