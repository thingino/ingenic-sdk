...
/* #define CONFIG_FPGA_ISP_V10 */

/* #ifndef CONFIG_FPGA_ISP_V10 */
/* #define CONFIG_FPGA_ISP_V9 */
/* #endif /1* CONFIG_FPGA_ISP_V10 *1/ */
...
enum tx_sensor_data_type {
	...
	TX_SENSOR_DATA_TYPE_WDR_DOL,    /**< Clear HDR / DOL HDR */
	TX_SENSOR_DATA_TYPE_WDR_NATIVE, /**< NATIVE / DAG */
};
...
typedef struct {
	...
	unsigned char pclk_polar;   /**< reserved */
} sensor_dvp_polar;
...
struct tx_isp_sensor_item {
	unsigned int value;
	unsigned int min;
	unsigned int max;
	unsigned short apply_delay;
};

struct tx_isp_sensor_hcg {
...
typedef struct tx_isp_sensor_ctrl{
	...
	unsigned int (*alloc_hcg)(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg);
	unsigned int (*alloc_hcg_short)(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg);
} TX_ISP_SENSOR_CTRL;
...
enum tx_sensor_expo_triggers {
	TX_SENSOR_EXPO_FRAME_DONE,
	TX_SENSOR_EXPO_FRAME_START
};

struct tx_isp_sensor_dag_attr {
	int enable;
	unsigned short curve_break[12];
};
...
struct tx_isp_sensor_attribute{
	...
	struct tx_isp_sensor_item again;
	struct tx_isp_sensor_item again_short;
	struct tx_isp_sensor_item dgain;
	struct tx_isp_sensor_item dgain_short;
	struct tx_isp_sensor_item integration_time;
	struct tx_isp_sensor_item integration_time_short;

	/* Read-only */
	unsigned int expo;
	unsigned int expo_short;

	enum tx_sensor_expo_triggers expo_fs;

	struct tx_isp_sensor_hcg hcg;
	struct tx_isp_sensor_hcg hcg_short;

	unsigned short total_width;
	unsigned short total_height;

	/* Read-only for Sensor drivers (format: us_den | (us_num << 16)) */
	unsigned int one_line_expr_in_us;
	...
	struct tx_isp_sensor_dag_attr dag_attr;
...
enum tx_isp_notification {
	...
	TX_ISP_EVENT_PM_S_STREAMON,
	TX_ISP_EVENT_G_NV12,
	TX_ISP_EVENT_WAIT_ALLDONE,
	TX_ISP_EVENT_CORE_ISP_RESET,
	TX_ISP_EVENT_VIC_FRAME_DROP,
	TX_ISP_EVENT_VIC_RESET,
	TX_ISP_EVENT_CORE_BAYER_CHANGE,
	TX_ISP_EVENT_VIC_SNAP_RAW,
	TX_ISP_EVENT_CORE_SNAP_RAW_SWITCH_MSCA,
	TX_ISP_EVENT_VIC_GPIO_STATE_SWITCH,
	TX_ISP_EVENT_VIC_GPIO_STATE_SWITCH_TRIGGER,
	TX_ISP_EVENT_ONESHOT_DISABLE,
	TX_ISP_EVENT_ONESHOT_ENABLE,
	TX_ISP_EVENT_ONESHOT_TICK,
	TX_ISP_EVENT_ONESHOT_WAKEUPTICK,
	TX_ISP_EVENT_ONESHOT_TICKTIMEOUT,
	TX_ISP_EVENT_ONESHOT_DROP_CNT,
	TX_ISP_EVENT_AE_ALGO_INIT,
	TX_ISP_EVENT_AE_ALGO_DEINIT,
	TX_ISP_EVENT_GET_AE_ALGO_INFO,
	TX_ISP_EVENT_SET_AE_ALGO_ATTR,
	TX_ISP_EVENT_AWB_ALGO_INIT,
	TX_ISP_EVENT_AWB_ALGO_DEINIT,
	TX_ISP_EVENT_GET_AWB_ALGO_INFO,
	TX_ISP_EVENT_SET_AWB_ALGO_ATTR,
	TX_ISP_EVENT_GET_SENSOR_INFO,
	TX_ISP_EVENT_SET_SWITCHBIN_DONE,
	TX_ISP_EVENT_SET_SWITCHBIN_CTRL,
	...
	TX_ISP_EVENT_SENSOR_HVFLIP,
	TX_ISP_EVENT_SENSOR_S_REGISTER,
	TX_ISP_EVENT_SENSOR_G_REGISTER,
	TX_ISP_EVENT_SENSOR_WDR_STOP,
	TX_ISP_EVENT_SENSOR_WDR_OPEN,
	TX_ISP_EVENT_SENSOR_START,
	TX_ISP_EVENT_SENSOR_DONE,
	TX_ISP_EVENT_SENSOR_EXPO,
	TX_ISP_EVENT_SENSOR_EXPO_SHORT,
	TX_ISP_EVENT_SENSOR_HCG,
	TX_ISP_EVENT_SENSOR_USER_DEF,/*aov define*/
	TX_ISP_EVENT_PM_S_STREAM_CTRL,/*aov define*/
	TX_ISP_EVENT_MULTI_MODE,
	TX_ISP_EVENT_BYPASS_MODE,
	TX_ISP_EVENT_SENSOR_DGAIN_SHORT,
	TX_ISP_EVENT_FRAME_SYNC_MODE,
	TX_ISP_EVENT_SENSOR_STREAM,
	...
	TX_ISP_EVENT_FRAME_CHAN_REQUEST_BUFFER,
	...
	/* the events of ldc are defined as follows. */
	TX_ISP_EVENT_LDC_ENABLE = NOTIFICATION_TYPE_LDC_OPS,
	TX_ISP_EVENT_LDC_DISABLE,
	TX_ISP_EVENT_LDC_SET_PARAMS,
	TX_ISP_EVENT_LDC_GET_CONTROL,
	TX_ISP_EVENT_LDC_SET_CONTROL,
	TX_ISP_EVENT_LDC_SET_MODE,
	TX_ISP_EVENT_LDC_ONLINE_INIT,
	TX_ISP_EVENT_LDC_GET_ONLINE_INIT_STATE,
	TX_ISP_EVENT_LDC_GET_ONLINE_PARAM,
	TX_ISP_EVENT_LDC_SET_ONLINE_HLDC_BYPASS,
	TX_ISP_EVENT_LDC_ONLINE_REFLESH_LUT,
};
...
struct tx_isp_wdr_par {
	int boot;
	int wdr_en;
	int exp_ratio;
};

enum tx_isp_hvflip_mode {
	TX_ISP_SENSOR_HVFLIP_NOMAL,
	TX_ISP_SENSOR_HVFLIP_HFLIP,
	TX_ISP_SENSOR_HVFLIP_VFLIP,
	TX_ISP_SENSOR_HVFLIP_HVFLIP
};

struct tx_isp_hvflip_par {
	enum tx_isp_hvflip_mode hvflip;
	int drop_frame;
	int reset;
};
...
struct tx_isp_sensor_subdev_id {
	int grp;
	int vic;
};
...
struct tx_isp_video_in {
	...
	struct tx_isp_sensor_item fps;
	struct tx_isp_sensor_subdev_id sd_id;
	int hvflip_mode;
	unsigned int wdr;
	int state;
};
...
typedef struct tx_isp_sensor_user_data{
	int dir;
	int cmd;
	int value;
}IMPISPUserCtl;
...
typedef enum{
	TX_SENSOR_PM_PREPARE = 0,
	TX_SENSOR_PM_SUSPEND,
	TX_SENSOR_PM_RESUME,
	TX_SENSOR_PM_COMPLETE,
	TX_SENSOR_PM_STREAMON,
	TX_SENSOR_PM_STREAMOFF,
} sensor_s_stream_type;

struct tx_isp_internal_arg {
	int vinum;
	int value;
};
