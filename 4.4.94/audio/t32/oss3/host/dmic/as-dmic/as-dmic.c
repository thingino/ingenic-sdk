...
struct ingenic_dmic *globe_dmic = NULL;
extern struct audio_dsp_debug dmic_debug;
#define BUFFER_SIZE_DMIC 256
...
static int audio_dmic_show(struct seq_file *m, void *v)
{
    int len = 0;
    struct dmic_device_info *info = NULL;
    struct ingenic_dmic *dmic = (struct ingenic_dmic*)(m->private);
...
static ssize_t dmic_debug_write(struct file *file, const char __user *user_buffer, size_t count, loff_t *ppos)
{
	int i = 0;
	char kbuffer[BUFFER_SIZE_DMIC];
	ssize_t bytes_read;
	if (count >= BUFFER_SIZE_DMIC) {
		printk("dmic_debug_write: input too large (%zu bytes)\n", count);
		return -EINVAL;
	}
	bytes_read = copy_from_user(kbuffer, user_buffer, count);
	if (bytes_read) {
		printk("dmic_debug_write: failed to copy from user space (%zd bytes)\n", bytes_read);
		return -EFAULT;
	}
	kbuffer[count] = '\0';
	if (strncasecmp(kbuffer, "reg", 3) == 0) {
		...
		printk("\nDump Dmic Regs:\n");
		for (i = 0  ; i <= 0x20; i+=4)
			printk("reg[%04x], value = %04x\n", i, readl(globe_dmic->io_base + i));
		printk("reg[%04x], value = %04x\n", 0x30, readl(globe_dmic->io_base + 0x30));
		printk("reg[%04x], value = %04x\n", 0x34, readl(globe_dmic->io_base + 0x34));
		printk("reg[%04x], value = %04x\n", 0x38, readl(globe_dmic->io_base + 0x38));
		printk("reg[%04x], value = %04x\n", 0x50, readl(globe_dmic->io_base + 0x50));
...
static struct proc_ops audio_dmic_debug_fops = {
	.proc_read = seq_read,
	.proc_open = dump_audio_dmic_open,
	.proc_write = dmic_debug_write,
...
#ifdef CONFIG_KERNEL_5_15
	pipe->vaddr = dma_alloc_noncoherent(dev, pipe->reservesize, &pipe->paddr, DMA_FROM_DEVICE, GFP_KERNEL);
#else
...
#ifdef CONFIG_KERNEL_5_15
		dma_free_noncoherent(dev, dmic->pipe->reservesize, dmic->pipe->vaddr, dmic->pipe->paddr, DMA_FROM_DEVICE);
#else
		dmam_free_noncoherent(dev, dmic->pipe->reservesize, dmic->pipe->vaddr, dmic->pipe->paddr);
#endif
...
static int set_dmic_gpio_func(struct jz_gpio_func_def gpio_func)
{
	...
	jzgpio_set_func(gpio_func.port, (enum gpio_function)gpio_func.func, gpio_func.pins);
...
static int ingenic_dmic_platform_probe(struct platform_device *pdev)
{
	...
	if (AUDIO_SUCCESS != register_audio_debug_ops("dmic_info", &audio_dmic_debug_fops, dmic)) {
