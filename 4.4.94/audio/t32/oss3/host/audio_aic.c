...
#define AUDIO_DRIVER_VERSION "V30-20250819a"
...
static struct audio_parameter micparam;
static struct volume gain_bk;
struct audio_aic_device *globe_aic = NULL;
extern struct audio_dsp_debug mic_debug;
extern struct audio_dsp_debug spk_debug;
extern struct audio_dsp_device* globe_dspdev;
extern int dump_time_stamp;
int codec_type = INNER_CODEC;
#define BUFFER_SIZE_AIC 256
int record_flag=0;
int play_flag=0;
...
static int extern_codec_register(struct audio_aic_device *aic, int i2c_adapter, char *name, int i2c_addr)
{
	...
	printk("excodec addr=0x%02x, excodec_name=%s\n", board_info.addr, board_info.type);
...
int inner_codec_register(struct codec_attributes *codec_attr)
{
	...
	if(globe_aic){
		...
		if (NULL != codec_attr->debug_ops){
			codec_file = proc_create("codec_info", 0, globe_dspdev->proc,
...
static int audio_aic_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "\nAI Info : \n");
	seq_printf(m, "samplerate : %lu\n", info->rate);
	seq_printf(m, "channel : %d\n", info->channel);
	seq_printf(m, "format : %d\n", info->format);
	seq_printf(m, "again : %d\n", info->lagain);
	seq_printf(m, "dgain : %d\n", info->ldgain);
	seq_printf(m, "frame rate : %d\n", mic_debug.frame_rate / 2);
	seq_printf(m, "frame interval : %lldms\n", mic_debug.time_diff_ms);
	seq_printf(m, "io tracer : %u\n", mic_debug.io_tracer_debug);
	seq_printf(m, "dma tracer : %u\n", mic_debug.dma_tracer_debug);
	seq_printf(m, "frame loss cnt : %u\n", mic_debug.io_late_cnt);
	...
	seq_printf(m, "again : %d\n", info->lagain);
	seq_printf(m, "dgain : %d\n", info->ldgain);
	seq_printf(m, "frame rate : %d\n", spk_debug.frame_rate / 2);
	seq_printf(m, "frame interval : %lldms\n", spk_debug.time_diff_ms);
	seq_printf(m, "io tracer : %u\n", spk_debug.io_tracer_debug);
	seq_printf(m, "dma tracer : %u\n", spk_debug.dma_tracer_debug);
...
static ssize_t audio_aic_debug_write(struct file *file, const char __user *user_buffer, size_t count, loff_t *ppos)
{
	char kbuffer[BUFFER_SIZE_AIC];
	ssize_t bytes_read;
	if (count >= BUFFER_SIZE_AIC) {
		printk("audio_aic_debug_write: input too large (%zu bytes)\n", count);
		return -EINVAL;
	}
	bytes_read = copy_from_user(kbuffer, user_buffer, count);
	if (bytes_read) {
		printk("audio_aic_debug_write: failed to copy from user space (%zd bytes)\n", bytes_read);
		return -EFAULT;
	}
	kbuffer[count] = '\0';
	if (strncasecmp(kbuffer, "reg", 3) == 0) {
		dump_aic_regs();
	}
	else if(strncasecmp(kbuffer, "timestamp_on", 12) == 0) {
		dump_time_stamp = 1;
	}
	else if(strncasecmp(kbuffer, "timestamp_no", 12) == 0) {
		dump_time_stamp = 0;
	}
...
#ifdef CONFIG_KERNEL_5_15
static struct proc_ops audio_aic_debug_fops = {
	.proc_read = seq_read,
	.proc_open = dump_audio_aic_open,
	.proc_write = audio_aic_debug_write,
...
static bool dma_chan_filter(struct dma_chan *chan, void *filter_param)
{
	...
	//printk("private:%d, type:%d\n", type, pipe->dma_type);
	return (unsigned int)pipe->dma_type == type;
}

static int pipe_request_dma(struct audio_pipe *pipe, struct device *dev, enum dma_data_direction dir)
{
	int ret = 0;
	dma_cap_mask_t mask;
...
#ifdef CONFIG_KERNEL_5_15
	pipe->vaddr = dma_alloc_noncoherent(dev, pipe->reservesize, &pipe->paddr, dir, GFP_KERNEL);
#else
...
#ifdef CONFIG_KERNEL_5_15
	dma_free_noncoherent(dev, pipe->reservesize, pipe->vaddr, pipe->paddr, dir);
#else
...
#ifdef CONFIG_KERNEL_5_15
			dma_free_noncoherent(dev, aic->mic_pipe->reservesize, aic->mic_pipe->vaddr, aic->mic_pipe->paddr, DMA_FROM_DEVICE);
#else
...
#ifdef CONFIG_KERNEL_5_15
			dma_free_noncoherent(dev, aic->spk_pipe->reservesize, aic->spk_pipe->vaddr, aic->spk_pipe->paddr, DMA_TO_DEVICE);
#else
...
#ifdef CONFIG_KERNEL_5_15
			dma_free_noncoherent(dev, aic->aec_pipe->reservesize, aic->aec_pipe->vaddr, aic->aec_pipe->paddr, DMA_FROM_DEVICE);
#else
...
static int set_codec_type(struct audio_aic_device *aic, void *data)
{
	int ret = 0;
	int codec = *(int *)data;
	if(codec_type == codec)
	...
	set_codec_mic_stop(aic,NULL);
	aic->livingcodec->set_power(CODEC_POWER_DOWN);
	aic->livingcodec->spk_gpio_free();
	if (EXTERNAL_CODEC == codec)
		aic->livingcodec = aic->excodec;
	else
		aic->livingcodec = aic->incodec;

	aic->livingcodec->spk_gpio_request();
	printk("the codec type is %d \n",codec);
	...
	set_codec_mic_datatype(aic, &micparam);
	set_codec_mic_start(aic, NULL);
	set_codec_mic_again(aic, &gain_bk);
	codec_type = codec;
	return 0;
}
...
static int mic_ioctl(void *pipe, unsigned int cmd, void *data)
{
	...
	switch(cmd) {
		...
		case AUDIO_CMD_SET_CODEC:
			ret = set_codec_type(aic, data);
			break;
...
static int init_dma_config(struct audio_aic_device *aic)
{
	...
	if (0 != pipe_request_dma(mic_pipe, aic->dev, DMA_FROM_DEVICE))
		goto failed_request_channel;
	if (0 != pipe_request_dma(spk_pipe, aic->dev, DMA_TO_DEVICE))
		goto failed_request_channel;
	if (0 != pipe_request_dma(aec_pipe, aic->dev, DMA_FROM_DEVICE))
		goto failed_request_channel;
...
struct platform_device *aic_device = NULL;
static int audio_aic_probe(struct platform_device *pdev)
{
...
#endif
#ifdef CONFIG_KERNEL_3_10
    aic->spk_clock = clk_get(&pdev->dev, "div_i2s_spk");
    if (IS_ERR(aic->spk_clock)) {
        audio_err_print("aic get i2s spk clock failed.\n");
        goto err_get_i2s_spk;
	 }
	aic->mic_clock = clk_get(&pdev->dev, "div_i2s_mic");
    if (IS_ERR(aic->mic_clock)) {
        audio_err_print("aic get i2s mic clock failed.\n");
        goto err_get_i2s_mic;
    }
    aic->aic_clock = clk_get(&pdev->dev, "gate_aic");
    if (IS_ERR(aic->aic_clock)) {
        audio_err_print("aic get aic clock failed.\n");
        goto err_get_aic_clk;
    }
	...
	if(excodec_addr != DEFAULT_EXCODEC_ADDR){
		printk("excodec name=%s, excodec_addr=0x%02x\n", excodec_name, excodec_addr);
	...
	if(codec_type == EXTERNAL_CODEC)
		aic->livingcodec = aic->excodec;
	else
		aic->livingcodec = aic->incodec;

	aic->livingcodec->spk_gpio_request();
	printk("the codec_type = %d !!!!!\n",codec_type);
	//enable clk
#if defined(CONFIG_KERNEL_4_4_94) || defined(CONFIG_KERNEL_5_15)
...
//		__i2s_select_msbjustified(aic);
	...
	if (AUDIO_SUCCESS != register_audio_debug_ops("audio_info", &audio_aic_debug_fops, aic)) {
...
void set_suspend_config(void)
{
	struct audio_aic_device *aic = globe_aic;
	struct platform_device  *pdev = aic_device;
	if(record_flag){
		__i2s_disable_record(aic);
		__i2s_disable_receive_dma(aic);
		__i2s_disable_tloop(aic);
		...
		__i2s_enable_stereo(aic);
		__i2s_disable(aic);
		aic->livingcodec->record->set_endpoint(CODEC_MIC_STOP);
	}
	if(play_flag){
		__i2s_disable_transmit_dma(aic);
		__i2s_disable_replay(aic);
		...
		msleep(2);
		__i2s_disable_replay(aic);
		aic->livingcodec->playback->set_endpoint(CODEC_SPK_STOP);
		//aic->livingcodec->spk_gpio_free();
	}
	clk_disable(aic->mic_clock);
	clk_disable(aic->spk_clock);
...
void dma_set_config(struct audio_pipe *pipe)
{
	dma_cap_mask_t mask;
	dma_cap_zero(mask);
	dma_cap_set(DMA_SLAVE, mask);
...
//	INIT_WORK(&delayed_work, codec_fast_start);
...
//#endif
	...
	//record
	if(record_flag){
	    clk_set_rate(aic->mic_clock, aic->codec_mic_info->rate*256);
		__i2s_enable_monoctr_left(aic);
		__i2s_set_iss_sample_size(aic, 1);
		__i2s_disable_signadj(aic);

		aic->livingcodec->record->set_datatype(&aic->codec_mic_info->data_type);
		//aic->livingcodec->set_sign(&aic->codec_mic_info->transfer_info);
		aic->livingcodec->record->set_again(MONO_LEFT,aic->codec_mic_info->lagain);
        __i2s_enable(aic);
		aic->livingcodec->record->set_endpoint(CODEC_MIC_START);
		__i2s_flush_rfifo(aic);
		__i2s_read_rfifo(aic);
		__i2s_enable_receive_dma(aic);
		__i2s_enable_record(aic);
	...
	//play
	if(play_flag){
	    clk_set_rate(aic->spk_clock, aic->codec_spk_info->rate*256);
		__i2s_out_channel_select(aic, 0x00);//one channel,only left
		__i2s_enable_mono2stereo(aic);
		__i2s_set_oss_sample_size(aic, 1);
		__i2s_disable_signadj(aic);
		__i2s_disable_byteswap(aic);
		aic->livingcodec->playback->set_datatype(&aic->codec_spk_info->data_type);
		//aic->livingcodec->set_sign(&aic->codec_spk_info->transfer_info);
		aic->livingcodec->playback->set_again(MONO_LEFT,aic->codec_spk_info->lagain);
		aic->livingcodec->playback->set_endpoint(CODEC_SPK_START);
		__i2s_flush_tfifo(aic);
		__i2s_enable_transmit_dma(aic);
		__i2s_enable_replay(aic);
	}
