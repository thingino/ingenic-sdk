...
unsigned int sc200ai_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
    ...
    while (lut->gain <= sc200ai_attr.again.max) {
        if (isp_gain == 0) {
            ...
            *sensor_again = (lut - 1)->value;
            ...
            if ((lut->gain == sc200ai_attr.again.max) && (isp_gain >= lut->gain)) {
...
unsigned int sc200ai_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
    ...
    /* Analog gain table */
    struct again_lut *lut = sc200ai_again_lut;
    while(lut->gain <= sc200ai_attr.again_short.max) {
        if(isp_gain == 0) {
            *sensor_again = 0;
            return 0;
        }
            ...
            *sensor_again = (lut - 1)->value;
            ...
            if((lut->gain == sc200ai_attr.again_short.max) && (isp_gain >= lut->gain)) {
                *sensor_again = lut->value;
                return lut->gain;
            }
        }
...
#ifdef SENSOR_HCG
unsigned int sc200ai_alloc_hcg(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
unsigned int sc200ai_alloc_hcg_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
struct tx_isp_dvp_bus sc200ai_dvp = {
...
static struct regval_list sc200ai_init_regs_1920_1080_30fps_mipi[] = {
    //cleaned_0x6b_SC200AI_MIPI_24Minput_783Mbps_1lane_10bit_1920x1080_30fps_non-continue_test
    {0x0103,0x01},
    {0x0100,0x00},
    {0x36e9,0x80},
    {0x36f9,0x80},
...
/*
 * the order of the sc200ai_win_sizes is [full_resolution, preview_resolution].
 */
static struct tx_isp_sensor_win_setting sc200ai_win_sizes[] = {
    {
        ...
        .regs           = sc200ai_init_regs_1920_1080_30fps_mipi,
    },
    {
        .width          = 1920,
        .height         = 1080,
...
static struct tx_isp_sensor_win_setting *wsize = &sc200ai_win_sizes[0];

/*
 * the part of driver was fixed.
 */

static struct regval_list sc200ai_stream_on_mipi[] = {
    {0x0100, 0x01},
    { SENSOR_REG_END, 0x00 }, /* END MARKER */
};

static struct regval_list sc200ai_stream_off_mipi[] = {
    {0x0100, 0x00},
    { SENSOR_REG_END, 0x00 }, /* END MARKER */
};

#ifdef SENSOR_I2C_REG_8BIT
int sc200ai_read(struct tx_isp_subdev *sd, unsigned char reg,
                 unsigned char *value)
{
...
int sc200ai_write(struct tx_isp_subdev *sd, unsigned char reg,
                  unsigned char value)
{
    struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int sc200ai_clk_set(struct tx_isp_subdev *sd, struct clk *sclka, int mclk)
{
...
static int sc200ai_attr_set(struct tx_isp_subdev *sd, struct tx_isp_sensor_win_setting *wise)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    int ret = ISP_SUCCESS;
...
static int sc200ai_setting_select(struct tx_isp_subdev *sd, int deboot)
{
    int ret = ISP_SUCCESS;
    ...
    case 0:
        ...
        sc200ai_attr.again.value = 65536;
        sc200ai_attr.again.max = 377102;
        sc200ai_attr.again.min = 0;
        sc200ai_attr.again.apply_delay = 2;

        sc200ai_attr.integration_time.value = 0xb60;
        sc200ai_attr.integration_time.max = 1125 - 4;
        sc200ai_attr.integration_time.min = 1;
        sc200ai_attr.integration_time.apply_delay = 2;

        sc200ai_attr.total_width = 1160;
        sc200ai_attr.total_height = 1125;

        sc200ai_attr.expo_fs = TX_SENSOR_EXPO_FRAME_START;

#ifdef SENSOR_HCG
        sc200ai_attr.hcg.base_gain = ;
        sc200ai_attr.hcg.apply_delay = ;
#endif /* SENSOR_HCG */

#ifdef SENSOR_WDR_2_FRAME
        sc200ai_attr.again_short.value = ;
        sc200ai_attr.again_short.max = ;
        sc200ai_attr.again_short.min = ;
        sc200ai_attr.again_short.apply_delay = ;

        sc200ai_attr.integration_time_short.value = ;
        sc200ai_attr.integration_time_short.max = ;
        sc200ai_attr.integration_time_short.min = ;
        sc200ai_attr.integration_time_short.apply_delay = ;

        sc200ai_attr.wdr_cache = wdr_line * sc200ai_attr.total_width;

#ifdef SENSOR_HCG
        sc200ai_attr.hcg_short.base_gain = ;
        sc200ai_attr.hcg_short.apply_delay = ;
#endif /* SENSOR_HCG */
#endif /* SENSOR_WDR_2_FRAME */

        memcpy((void *)(&(sc200ai_attr.mipi)), (void *)(&sc200ai_mipi_linear), sizeof(sc200ai_attr.mipi));
        break;
    case 1:
        ...
        sc200ai_attr.integration_time.value = 0xb60;
        sc200ai_attr.integration_time.max = 1200 - 4;
        sc200ai_attr.integration_time.min = 1;
        sc200ai_attr.integration_time.apply_delay = 2;

        sc200ai_attr.total_width = 1100;
...
static int sc200ai_attr_check(struct tx_isp_subdev *sd)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    ...
    sc200ai_setting_select(sd, info->default_boot);
    ...
    ret = sc200ai_clk_set(sd, sclka, SENSOR_MCLK);
    if (ret) {
                                ...
                                struct tx_isp_chip_ident *chip)
{
    ...
    sc200ai_attr_check(sd);
#ifndef ZRT_SENSOR_WITHOUT_INIT
    if (info->rst_gpio != -1) {
        ret = private_gpio_request(info->rst_gpio, "sc200ai_reset");
        if (!ret) {
    ...
    if (info->pwdn_gpio != -1) {
        ret = private_gpio_request(info->pwdn_gpio, "sc200ai_pwdn");
        if (!ret) {
...
static int sc200ai_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
    ...
    if (init->enable) {
        ...
        if (sensor->video.state == TX_ISP_MODULE_INIT) {
            ret = sc200ai_write_array(sd, sc200ai_stream_on_mipi);
            sensor->video.state = TX_ISP_MODULE_RUNNING;
            pr_debug("sc200ai stream on\n");
        }

    } else {
        ret = sc200ai_write_array(sd, sc200ai_stream_off_mipi);
        sensor->video.state = TX_ISP_MODULE_INIT;
...
static int sc200ai_set_expo(struct tx_isp_subdev *sd, int value)
{
    ...
    it = (it << 1) - 1;
    ret += sc200ai_write(sd, 0x3e00, (unsigned char)((it >> 12) & 0xf));
    ret += sc200ai_write(sd, 0x3e01, (unsigned char)((it >> 4) & 0xff));
    ret += sc200ai_write(sd, 0x3e02, (unsigned char)((it & 0xf) << 4));
...
static int sc200ai_set_integration_time(struct tx_isp_subdev *sd, int value)
{
    ...
    value = (value << 1) - 1;
    ret += sc200ai_write(sd, 0x3e00, (unsigned char)((value >> 12) & 0xf));
    ret += sc200ai_write(sd, 0x3e01, (unsigned char)((value >> 4) & 0xff));
    ret += sc200ai_write(sd, 0x3e02, (unsigned char)((value & 0xf) << 4));
...
static int sc200ai_set_hvflip(struct tx_isp_subdev *sd, void *arg)
{
...
static int sc200ai_pm_s_stream_ctrl(struct tx_isp_subdev *sd, void *arg)
{
    int ret = ISP_SUCCESS;
    ...
    if(data->cmd == TX_SENSOR_PM_RESUME){
        ...
        printk("sc200ai TX_SENSOR_PM_RESUME\n");
        ...
        printk("sc200ai TX_SENSOR_PM_SUSPEND !!!\n");
    }else if(data->cmd == TX_SENSOR_PM_PREPARE) {
        printk("\n sc200ai TX_SENSOR_PM_PREPARE !!!\n");
        ...
        printk("\n sc200ai TX_SENSOR_PM_STREAMOFF !!!\n");
    }else {
...
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
static int sc200ai_set_expo_short(struct tx_isp_subdev *sd, int value)
{
    int ret = ISP_SUCCESS;
...
static int sc200ai_set_wdr_stop(struct tx_isp_subdev *sd, void *arg)
{
    struct tx_isp_sensor *sensor = tx_isp_get_subdev_hostdata(sd);
    struct tx_isp_sensor_register_info *info = &sensor->info;
    ...
    if (par->wdr_en == 1) {
        ...
        sc200ai_setting_select(sd, 1);
        sc200ai_attr_set(sd, wsize);
    } else if (par->wdr_en == 0) {
        if (par->boot == -1) {
            info->default_boot = 0;
        }
        sc200ai_setting_select(sd, 0);
        sc200ai_attr_set(sd, wsize);
    } else {
...
static int sc200ai_set_wdr(struct tx_isp_subdev *sd, void *arg)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    struct tx_isp_wdr_par *par = (struct tx_isp_wdr_par *)arg;
    struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int sc200ai_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    case TX_ISP_EVENT_SENSOR_HVFLIP:
        if(arg)
            ret = sc200ai_set_hvflip(sd, (void *)sensor_val->value);
        break;
    case TX_ISP_EVENT_PM_S_STREAM_CTRL:
        if(arg)
            ret =  sc200ai_pm_s_stream_ctrl(sd, arg);
        break;
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
    case TX_ISP_EVENT_SENSOR_EXPO_SHORT:
        if (arg)
            ret = sc200ai_set_expo_short(sd, sensor_val->value);
        break;
#else
    ...
    case TX_ISP_EVENT_SENSOR_WDR:
        if(arg)
            ret = sc200ai_set_wdr(sd, (void *)sensor_val->value);
        break;
    case TX_ISP_EVENT_SENSOR_WDR_STOP:
        if(arg)
            ret = sc200ai_set_wdr_stop(sd, (void *)sensor_val->value);
        break;
#endif /* SENSOR_WDR_2_FRAME */
    default:
        break;
...
#ifdef CONFIG_ZERATUL
static int init_first_sc200ai(void) {
    return private_i2c_add_driver(&sc200ai_driver);
}

static void exit_first_sc200ai(void) {
    private_i2c_del_driver(&sc200ai_driver);
}
...
struct tx_isp_sensor_fast_attr sensor0 = {
    ...
    .wdr = 0,
#endif /* SENSOR_WDR_2_FRAME */
    .init_sensor = init_first_sc200ai,
    .exit_sensor = exit_first_sc200ai
};
#endif  /* CONFIG_ZERATUL */

MODULE_DESCRIPTION("A low-level driver for sc200ai sensor");
MODULE_LICENSE("GPL");
