 ...
 *  0           1920*1080       30        mipi_2lane    linear  12        30
 * @I2C addr:0x3c, 0x3d
 *
 * @FSync:none
 *
...
unsigned int os02h10_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	while (lut->gain <= os02h10_attr.again.max) {
		if (isp_gain == 0) {
			...
			*sensor_again = (lut - 1)->value;
			...
			if ((lut->gain == os02h10_attr.again.max) && (isp_gain >= lut->gain)) {
...
#ifdef SENSOR_WDR_2_FRAME
unsigned int os02h10_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
#ifndef SENSOR_TEST
#ifdef SENSOR_AGAIN_TABLE
	/* Analog gain table */
	struct again_lut *lut = os02h10_again_lut;
	while(lut->gain <= os02h10_attr.max_again_short) {
		if(isp_gain == 0) {
			*sensor_again = 0;
			return 0;
		}
			...
			*sensor_again = (lut - 1)->value;
			...
			if((lut->gain == os02h10_attr.max_again_short) && (isp_gain >= lut->gain)) {
				*sensor_again = lut->value;
				return lut->gain;
			}
		}
...
#ifdef SENSOR_HCG
unsigned int os02h10_alloc_hcg(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
unsigned int os02h10_alloc_hcg_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
struct tx_isp_mipi_bus os02h10_mipi_linear = {
...
struct tx_isp_dvp_bus os02h10_dvp = {
...
#ifdef SENSOR_WDR_2_FRAME
	.sensor_ctrl.alloc_again_short = os02h10_alloc_again_short,
#endif /* SENSOR_WDR_2_FRAME */
...
static struct tx_isp_sensor_win_setting *wsize = &os02h10_win_sizes[0];
...
int os02h10_read(struct tx_isp_subdev *sd, uint16_t reg,
		 unsigned char *value) {
	int ret;
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int os02h10_write(struct tx_isp_subdev *sd, uint16_t reg,
		  unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int os02h10_clk_set(struct tx_isp_subdev *sd, struct clk *sclka, int mclk)
{
...
static int os02h10_attr_set(struct tx_isp_subdev *sd, struct tx_isp_sensor_win_setting *wise)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
...
static int os02h10_setting_select(struct tx_isp_subdev *sd, int deboot)
{
	int ret = ISP_SUCCESS;
	...
	case 0:
		...
		os02h10_attr.again.value = 65536;
		os02h10_attr.again.max = 259142;
		os02h10_attr.again.min = 0;
		os02h10_attr.again.apply_delay = 2;

		os02h10_attr.integration_time.value = 0x7b6;
		os02h10_attr.integration_time.max = 2250 - 4;
		os02h10_attr.integration_time.min = 1;
		os02h10_attr.integration_time.apply_delay = 2;

		os02h10_attr.total_width = 1244;
		os02h10_attr.total_height = 2250;

		os02h10_attr.expo_fs = TX_SENSOR_EXPO_FRAME_START;

#ifdef SENSOR_HCG
		os02h10_attr.hcg.base_gain = ;
		os02h10_attr.hcg.apply_delay = ;
#endif /* SENSOR_HCG */

#ifdef SENSOR_WDR_2_FRAME
		os02h10_attr.again_short.value = ;
		os02h10_attr.again_short.max = ;
		os02h10_attr.again_short.min = ;
		os02h10_attr.again_short.apply_delay = ;

		os02h10_attr.integration_time_short.value = ;
		os02h10_attr.integration_time_short.max = ;
		os02h10_attr.integration_time_short.min = ;
		os02h10_attr.integration_time_short.apply_delay = ;

		os02h10_attr.wdr_cache = wdr_line * os02h10_attr.total_width;

#ifdef SENSOR_HCG
		os02h10_attr.hcg_short.base_gain = ;
		os02h10_attr.hcg_short.apply_delay = ;
#endif /* SENSOR_HCG */
#endif /* SENSOR_WDR_2_FRAME */

		memcpy((void *)(&(os02h10_attr.mipi)), (void *)(&os02h10_mipi_linear), sizeof(os02h10_attr.mipi));
		break;
	default:
...
static int os02h10_attr_check(struct tx_isp_subdev *sd)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	...
	os02h10_setting_select(sd, info->default_boot);
	...
	ret = os02h10_clk_set(sd, sclka, SENSOR_MCLK);
	if (ret) {
				...
				struct tx_isp_chip_ident *chip)
{
	...
	ISP_WARNING("===================================================\n");
	ISP_WARNING("OS02H10 version is %s\n", TVERSION);
...
static int os02h10_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			...
			pr_debug("os02h10 stream on\n");
		}

	} else {
		ret = os02h10_write_array(sd, os02h10_stream_off_mipi);
		sensor->video.state = TX_ISP_MODULE_INIT;
		pr_debug("os02h10 stream off\n");
	}
...
static int os02h10_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	ret += os02h10_write(sd, 0x03, (unsigned char)((it >> 8) & 0xff));
	ret += os02h10_write(sd, 0x04, (unsigned char)(it & 0xff));
...
static int os02h10_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	...
	ret = os02h10_write(sd, 0xfd, 0x01);
	ret += os02h10_write(sd, 0x03, (unsigned char)((value >> 8) & 0xff));
	ret += os02h10_write(sd, 0x04, (unsigned char)(value & 0xff));
	ret += os02h10_write(sd, 0x01, 0x01);
...
static int os02h10_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	ret = os02h10_write(sd, 0xfd, 0x01);
	ret += os02h10_write(sd, 0x24, (unsigned char)(value & 0xff));
	ret += os02h10_write(sd, 0x01, 0x01);
...
static int os02h10_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	hts |= val;
	if (0 != ret) {
		ISP_ERROR("err: os02h10 read err\n");
		return ret;
	}
	...
	ret += os02h10_write(sd, 0xfd, 0x01);
	os02h10_write(sd, 0x06, (unsigned char) (vb & 0xff));
	os02h10_write(sd, 0x05, (unsigned char) (vb >> 8));
	ret += os02h10_write(sd, 0x01, 0x01);
...
static int os02h10_set_hvflip(struct tx_isp_subdev *sd, void *arg)
{
...
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
static int os02h10_set_expo_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
#else
static int os02h10_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
static int os02h10_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
static int os02h10_set_wdr_stop(struct tx_isp_subdev *sd, void *arg)
{
	...
	if (par->wdr_en == 1) {
		...
		os02h10_setting_select(sd, 1);
		os02h10_attr_set(sd, wsize);
	} else if (par->wdr_en == 0) {
		if (par->boot == -1) {
			info->default_boot = 0;
		}
		os02h10_setting_select(sd, 0);
		os02h10_attr_set(sd, wsize);
	} else {
...
static int os02h10_set_wdr(struct tx_isp_subdev *sd, void *arg)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	struct tx_isp_wdr_par *par = (struct tx_isp_wdr_par *)arg;
	struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int os02h10_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_HVFLIP:
		if(arg)
			ret = os02h10_set_hvflip(sd, (void *)sensor_val->value);
		break;
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
	case TX_ISP_EVENT_SENSOR_EXPO_SHORT:
		if (arg)
			ret = os02h10_set_expo_short(sd, sensor_val->value);
		break;
#else
	case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
		if(arg)
			ret = os02h10_set_integration_time_short(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
		if(arg)
			ret = os02h10_set_analog_gain_short(sd, sensor_val->value);
		break;
#endif /* SENSOR_EXPO */
	case TX_ISP_EVENT_SENSOR_WDR:
		if(arg)
			ret = os02h10_set_wdr(sd, (void *)sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_WDR_STOP:
		if(arg)
			ret = os02h10_set_wdr_stop(sd, (void *)sensor_val->value);
		break;
#endif /* SENSOR_WDR_2_FRAME */
	default:
		break;
...
static int os02h10_probe(struct i2c_client *client,
...
#ifdef CONFIG_ZERATUL
static int init_first_os02h10(void) {
	return private_i2c_add_driver(&os02h10_driver);
}

static void exit_first_os02h10(void) {
	private_i2c_del_driver(&os02h10_driver);
}
...
struct tx_isp_sensor_fast_attr sensor0 = {
	...
	.wdr = 0,
#endif /* SENSOR_WDR_2_FRAME */
	.init_sensor = init_first_os02h10,
	.exit_sensor = exit_first_os02h10
};
#endif	/* CONFIG_ZERATUL */

MODULE_DESCRIPTION("A low-level driver for Sony os02h10 sensor");
MODULE_LICENSE("GPL");
