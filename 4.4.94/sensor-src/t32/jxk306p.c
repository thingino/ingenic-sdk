 ...
 *   0          2560x1440       30          MIPI        Linear   10      30
 * @I2C addr:
 *
 * @FSync:
 *
...
#define SENSOR_VERSION  "H20250401a"
...
unsigned int jxk306p_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	while (lut->gain <= jxk306p_attr.again.max) {
		if (isp_gain == 0) {
			...
			*sensor_again = (lut - 1)->value;
			...
			if ((lut->gain == jxk306p_attr.again.max) && (isp_gain >= lut->gain)) {
...
#ifdef SENSOR_WDR_2_FRAME
unsigned int jxk306p_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
#ifndef SENSOR_TEST
#ifdef SENSOR_AGAIN_TABLE
	/* Analog gain table */
	struct again_lut *lut = jxk306p_again_lut;
	while(lut->gain <= jxk306p_attr.again_short.max) {
		if(isp_gain == 0) {
			*sensor_again = 0;
			return 0;
		}
			...
			*sensor_again = (lut - 1)->value;
			...
			if((lut->gain == jxk306p_attr.again_short.max) && (isp_gain >= lut->gain)) {
				*sensor_again = lut->value;
				return lut->gain;
			}
		}
...
#ifdef SENSOR_HCG
unsigned int jxk306p_alloc_hcg(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
unsigned int jxk306p_alloc_hcg_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
struct tx_isp_mipi_bus jxk306p_mipi_linear = {
...
struct tx_isp_dvp_bus jxk306p_dvp = {
...
#ifdef SENSOR_WDR_2_FRAME
	.sensor_ctrl.alloc_again_short = jxk306p_alloc_again_short,
#endif /* SENSOR_WDR_2_FRAME */
...
static struct regval_list jxk306p_init_regs_2880_1620_30fps_mipi[] = {
	//[K306P(AC)_007_20240408_MIPI10_2L_2560x1440x30_M24_P172P8_F3840x1500_H4]
	//Width=2560
	//Height=1440
	//FrameWidth=3840
	//FrameHeight=1500
	...
	{0x5F, 0x41},
	{0x60, 0x20},
	{0x20, 0xC0},//hts = 0x3c0 = 960
	{0x21, 0x03},//
	{0x22, 0xDC},//vts = 0x5dc = 1500
	{0x23, 0x05},//
	{0x24, 0x80},
	{0x25, 0xA0},
	{0x26, 0x52},
...
static struct tx_isp_sensor_win_setting jxk306p_win_sizes[] = {
	/* 2560*1440 [0] */
	{
		...
		.regs           = jxk306p_init_regs_2880_1620_30fps_mipi,
	},
};

static struct tx_isp_sensor_win_setting *wsize = &jxk306p_win_sizes[0];
...
int jxk306p_read(struct tx_isp_subdev *sd, uint16_t reg,
		 unsigned char *value) {
	int ret;
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int jxk306p_write(struct tx_isp_subdev *sd, uint16_t reg,
		  unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int jxk306p_clk_set(struct tx_isp_subdev *sd, struct clk *sclka, int mclk)
{
...
static int jxk306p_attr_set(struct tx_isp_subdev *sd, struct tx_isp_sensor_win_setting *wise)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
...
static int jxk306p_setting_select(struct tx_isp_subdev *sd, int deboot)
{
	int ret = ISP_SUCCESS;
	...
	case 0:
		...
		jxk306p_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;

		jxk306p_attr.again.value = 0x64;
		jxk306p_attr.again.max = 259142;
		jxk306p_attr.again.min = 0;
		jxk306p_attr.again.apply_delay = 2;

		jxk306p_attr.integration_time.value = 1500 - 4 ;
		jxk306p_attr.integration_time.max = 1500 - 4;
		jxk306p_attr.integration_time.min = 1;
		jxk306p_attr.integration_time.apply_delay = 2;

		jxk306p_attr.total_width = 3840;
		jxk306p_attr.total_height = 1500;

		jxk306p_attr.expo_fs = 1;

#ifdef SENSOR_HCG
		jxk306p_attr.hcg.base_gain = ;
		jxk306p_attr.hcg.apply_delay = ;
#endif /* SENSOR_HCG */

#ifdef SENSOR_WDR_2_FRAME
		jxk306p_attr.again_short.value = ;
		jxk306p_attr.again_short.max = ;
		jxk306p_attr.again_short.min = ;
		jxk306p_attr.again_short.apply_delay = ;

		jxk306p_attr.integration_time_short.value = ;
		jxk306p_attr.integration_time_short.max = ;
		jxk306p_attr.integration_time_short.min = ;
		jxk306p_attr.integration_time_short.apply_delay = ;

		jxk306p_attr.wdr_cache = wdr_line * jxk306p_attr.total_width;

#ifdef SENSOR_HCG
		jxk306p_attr.hcg_short.base_gain = ;
		jxk306p_attr.hcg_short.apply_delay = ;
#endif /* SENSOR_HCG */
#endif /* SENSOR_WDR_2_FRAME */

		memcpy((void *)(&(jxk306p_attr.mipi)), (void *)(&jxk306p_mipi_linear), sizeof(jxk306p_attr.mipi));
		break;
	default:
...
static int jxk306p_attr_check(struct tx_isp_subdev *sd)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	...
	jxk306p_setting_select(sd, info->default_boot);

	switch (info->video_interface) {
	case TISP_SENSOR_VI_MIPI_CSI0:
		jxk306p_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		jxk306p_attr.mipi.index = 0;
		break;
	case TISP_SENSOR_VI_MIPI_CSI1:
		jxk306p_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		jxk306p_attr.mipi.index = 1;
		break;
	case TISP_SENSOR_VI_DVP:
		jxk306p_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_DVP;
		break;
	default:
	...
	ret = jxk306p_clk_set(sd, sclka, SENSOR_MCLK);
	if (ret) {
				...
				struct tx_isp_chip_ident *chip)
{
	...
	jxk306p_attr_check(sd);
#ifndef ZRT_SENSOR_WITHOUT_INIT
	if (info->rst_gpio != -1) {
		ret = private_gpio_request(info->rst_gpio, "jxk306p_reset");
		if (!ret) {
	...
	if (info->pwdn_gpio != -1) {
		ret = private_gpio_request(info->pwdn_gpio, "jxk306p_pwdn");
		if (!ret) {
	...
	ISP_WARNING("===================================================\n");
	ISP_WARNING("jxk306p version is %s\n", TVERSION);
...
static int jxk306p_reset(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	return 0;
}

static int jxk306p_init(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
...
static int jxk306p_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			...
			pr_debug("jxk306p stream on\n");
		}

	} else {
		ret = jxk306p_write_array(sd, jxk306p_stream_off_mipi);
		sensor->video.state = TX_ISP_MODULE_INIT;
		pr_debug("jxk306p stream off\n");
	}
...
static int jxk306p_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	ret += jxk306p_write(sd, 0x00, (unsigned char)(again & 0x3f));
...
static int jxk306p_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	jxk306p_write(sd, 0x22, (unsigned char) (vts & 0xff));
	jxk306p_write(sd, 0x23, (unsigned char) (vts >> 8));
...
static int jxk306p_set_hvflip(struct tx_isp_subdev *sd, void *arg)
{
...
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
static int jxk306p_set_expo_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
#else
static int jxk306p_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
static int jxk306p_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
static int jxk306p_set_wdr_stop(struct tx_isp_subdev *sd, void *arg)
{
	struct tx_isp_sensor *sensor = tx_isp_get_subdev_hostdata(sd);
	struct tx_isp_sensor_register_info *info = &sensor->info;
	...
	if (par->wdr_en == 1) {
		...
		jxk306p_setting_select(sd, 1);
		jxk306p_attr_set(sd, wsize);
	} else if (par->wdr_en == 0) {
		if (par->boot == -1) {
			info->default_boot = 0;
		}
		jxk306p_setting_select(sd, 0);
		jxk306p_attr_set(sd, wsize);
	} else {
...
static int jxk306p_set_wdr(struct tx_isp_subdev *sd, void *arg)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	struct tx_isp_wdr_par *par = (struct tx_isp_wdr_par *)arg;
	struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int jxk306p_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if (arg)
			ret = jxk306p_set_expo(sd, sensor_val->value);
		break;
#else
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if (arg)
			ret = jxk306p_set_integration_time(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if (arg)
			ret = jxk306p_set_analog_gain(sd, sensor_val->value);
		break;
#endif /* SENSOR_EXPO */
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if (arg)
			ret = jxk306p_set_digital_gain(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if (arg)
			ret = jxk306p_get_black_pedestal(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if (arg)
			ret = jxk306p_set_mode(sd, sensor_val->value);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if (arg)
			ret = jxk306p_set_fps(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_HVFLIP:
		if(arg)
			ret = jxk306p_set_hvflip(sd, (void *)sensor_val->value);
		break;
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
	case TX_ISP_EVENT_SENSOR_EXPO_SHORT:
		if (arg)
			ret = jxk306p_set_expo_short(sd, sensor_val->value);
		break;
#else
	case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
		if(arg)
			ret = jxk306p_set_integration_time_short(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
		if(arg)
			ret = jxk306p_set_analog_gain_short(sd, sensor_val->value);
		break;
#endif /* SENSOR_EXPO */
	case TX_ISP_EVENT_SENSOR_WDR:
		if(arg)
			ret = jxk306p_set_wdr(sd, (void *)sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_WDR_STOP:
		if(arg)
			ret = jxk306p_set_wdr_stop(sd, (void *)sensor_val->value);
		break;
#endif /* SENSOR_WDR_2_FRAME */
	default:
		break;
...
static int jxk306p_probe(struct i2c_client *client,
...
#ifdef CONFIG_ZERATUL
static int init_first_jxk306p(void) {
	return private_i2c_add_driver(&jxk306p_driver);
}

static void exit_first_jxk306p(void) {
	private_i2c_del_driver(&jxk306p_driver);
}
...
struct tx_isp_sensor_fast_attr sensor0 = {
	...
	.wdr = 0,
#endif /* SENSOR_WDR_2_FRAME */
	.init_sensor = init_first_jxk306p,
	.exit_sensor = exit_first_jxk306p
};
#endif  /* CONFIG_ZERATUL */

MODULE_DESCRIPTION("A low-level driver for jxk306p sensor");
MODULE_LICENSE("GPL");
