 ...
 *  0       2304*1290   30      mipi_2lane  linear  10      30          27M
 *  0       2304*1290   30      mipi_2lane  linear  10      30          24M
 *
 * @I2C addr:0x30, 0x32
 *
 * @FSync:none
...
#define SENSOR_MCLK_BOOT1 24000000
...
struct again_lut sc3336_again_lut[] = {
	{0x80, 0},
	{0x84, 2886},
	{0x88, 5776},
	{0x8c, 8494},
...
#ifdef SENSOR_AGAIN_TABLE
    /* Analog gain table */
    struct again_lut *lut = sc3336_again_lut;
    while (lut->gain <= sc3336_attr.again.max) {
        if (isp_gain == 0) {
            ...
            *sensor_again = (lut - 1)->value;
            ...
            if ((lut->gain == sc3336_attr.again.max) && (isp_gain >= lut->gain)) {
...
#ifdef SENSOR_WDR_2_FRAME
unsigned int sc3336_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
#ifndef SENSOR_TEST
#ifdef SENSOR_AGAIN_TABLE
    /* Analog gain table */
    struct again_lut *lut = sc3336_again_lut;
    while(lut->gain <= sc3336_attr.again_short.max) {
        if(isp_gain == 0) {
            *sensor_again = 0;
            return 0;
        }
            ...
            *sensor_again = (lut - 1)->value;
            ...
            if((lut->gain == sc3336_attr.again_short.max) && (isp_gain >= lut->gain)) {
                *sensor_again = lut->value;
                return lut->gain;
            }
        }
...
#ifdef SENSOR_HCG
unsigned int sc3336_alloc_hcg(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
unsigned int sc3336_alloc_hcg_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
struct tx_isp_mipi_bus sc3336_mipi_linear = {
...
struct tx_isp_dvp_bus sc3336_dvp = {
...
#ifdef SENSOR_WDR_2_FRAME
    .sensor_ctrl.alloc_again_short = sc3336_alloc_again_short,
#endif /* SENSOR_WDR_2_FRAME */
...
static struct regval_list sc3336_init_regs_2304_1296_30fps_24M_mipi[] = {
    //cleaned_0x02_SC3336_MIPI_24Minput_2Lane_10bit_510Mbps_2304x1296_30fps
    {0x0103,0x01},
    {0x36e9,0x80},
    {0x37f9,0x80},
    {0x301f,0x02},
...
/*
 * the order of the sc3336_win_sizes is [full_resolution, preview_resolution].
 */
static struct tx_isp_sensor_win_setting sc3336_win_sizes[] = {
    ...
    },
    {
        ...
        .mbus_code      = TISP_VI_FMT_SBGGR10_1X10,
        .colorspace     = TISP_COLORSPACE_SRGB,
        .regs           = sc3336_init_regs_2304_1296_30fps_24M_mipi,
    },
};

static struct tx_isp_sensor_win_setting *wsize = &sc3336_win_sizes[0];
...
#ifdef SENSOR_I2C_REG_8BIT
int sc3336_read(struct tx_isp_subdev *sd, unsigned char reg,
                  unsigned char *value)
{
...
int sc3336_write(struct tx_isp_subdev *sd, unsigned char reg,
                   unsigned char value)
{
    struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int sc3336_read(struct tx_isp_subdev *sd, uint16_t reg,
                  unsigned char *value) {
    int ret;
    struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int sc3336_write(struct tx_isp_subdev *sd, uint16_t reg,
                   unsigned char value)
{
    struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int sc3336_clk_set(struct tx_isp_subdev *sd, struct clk *sclka, int mclk)
{
...
static int sc3336_attr_set(struct tx_isp_subdev *sd, struct tx_isp_sensor_win_setting *wise)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    int ret = ISP_SUCCESS;
...
static int sc3336_setting_select(struct tx_isp_subdev *sd, int deboot)
{
    int ret = ISP_SUCCESS;
    ...
    case 0:
        ...
        sc3336_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;

        sc3336_attr.again.value = 65536;
        sc3336_attr.again.max = 365791;
        sc3336_attr.again.min = 0;
        sc3336_attr.again.apply_delay = 2;

        sc3336_attr.integration_time.value = 0xb60;
        sc3336_attr.integration_time.max = 1350 - 8;
        sc3336_attr.integration_time.min = 2;
        sc3336_attr.integration_time.apply_delay = 2;

        sc3336_attr.total_width = 1250;
        sc3336_attr.total_height = 1350;

        sc3336_attr.expo_fs = TX_SENSOR_EXPO_FRAME_START;

#ifdef SENSOR_HCG
        sc3336_attr.hcg.base_gain = ;
        sc3336_attr.hcg.apply_delay = ;
#endif /* SENSOR_HCG */

#ifdef SENSOR_WDR_2_FRAME
        sc3336_attr.again_short.value = ;
        sc3336_attr.again_short.max = ;
        sc3336_attr.again_short.min = ;
        sc3336_attr.again_short.apply_delay = ;

        sc3336_attr.integration_time_short.value = ;
        sc3336_attr.integration_time_short.max = ;
        sc3336_attr.integration_time_short.min = ;
        sc3336_attr.integration_time_short.apply_delay = ;

        sc3336_attr.wdr_cache = wdr_line * sc3336_attr.total_width;

#ifdef SENSOR_HCG
        sc3336_attr.hcg_short.base_gain = ;
        sc3336_attr.hcg_short.apply_delay = ;
#endif /* SENSOR_HCG */
#endif /* SENSOR_WDR_2_FRAME */

        memcpy((void *)(&(sc3336_attr.mipi)), (void *)(&sc3336_mipi_linear), sizeof(sc3336_attr.mipi));
        break;
    case 1:
        wsize = &sc3336_win_sizes[1];
        sc3336_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
        ...
        sc3336_attr.integration_time.value = 0xb60;
        sc3336_attr.integration_time.max = 1360 - 8;
        sc3336_attr.integration_time.min = 2;
        sc3336_attr.integration_time.apply_delay = 2;
...
static int sc3336_attr_check(struct tx_isp_subdev *sd)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    ...
    sc3336_setting_select(sd, info->default_boot);
    ...
    case 0:
        ret = sc3336_clk_set(sd, sclka, SENSOR_MCLK);
        if (ret) {
            ISP_ERROR("MCLK configuration failed!!!\n");
        }
        break;
    case 1:
        ret = sc3336_clk_set(sd, sclka, SENSOR_MCLK_BOOT1);
        if (ret) {
            ISP_ERROR("MCLK configuration failed!!!\n");
        }
        break;
                                 ...
                                 struct tx_isp_chip_ident *chip)
{
    ...
    sc3336_attr_check(sd);
#ifndef ZRT_SENSOR_WITHOUT_INIT
    if (info->rst_gpio != -1) {
        ret = private_gpio_request(info->rst_gpio, "sc3336_reset");
        if (!ret) {
    ...
    if (info->pwdn_gpio != -1) {
        ret = private_gpio_request(info->pwdn_gpio, "sc3336_pwdn");
        if (!ret) {
...
static int sc3336_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
    ...
    if (init->enable) {
        ...
        if (sensor->video.state == TX_ISP_MODULE_INIT) {
            ...
            pr_debug("sc3336 stream on\n");
        }

    } else {
        ret = sc3336_write_array(sd, sc3336_stream_off_mipi);
        sensor->video.state = TX_ISP_MODULE_INIT;
        pr_debug("sc3336 stream off\n");
    }
...
static int sc3336_set_expo(struct tx_isp_subdev *sd, int value)
{
    ...
    ret += sc3336_write(sd, 0x3e02, (unsigned char)((it & 0xf) << 4));

    ret += sc3336_write(sd, 0x3e09, (unsigned char)((again >> 8) & 0xff));
    ret += sc3336_write(sd, 0x3e07, (unsigned char)(again & 0xff));
...
static int sc3336_set_integration_time(struct tx_isp_subdev *sd, int value)
{
    ...
    ret += sc3336_write(sd, 0x3e02, (unsigned char)((value & 0xf) << 4));
...
static int sc3336_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
    ...
    ret += sc3336_write(sd, 0x3e09, (unsigned char)((value >> 8) & 0xff));
    ret += sc3336_write(sd, 0x3e07, (unsigned char)(value & 0xff));
...
static int sc3336_set_fps(struct tx_isp_subdev *sd, int fps)
{
    ...
    switch (info->default_boot) {
    case 0:
        sclk = 1250 * 1350 * 30;  /**< HTS * VTS * FPS */
        max_fps = 30;
        break;
    case 1:
        sclk = 1250 * 1360 * 30;  /**< HTS * VTS * FPS */
        max_fps = 30;
        break;
    default:
        ret = -1;
    ...
    val = 0;
    ret += sc3336_read(sd, 0x320c, &val);
    hts = val << 8;
    val = 0;
    ret += sc3336_read(sd, 0x320d, &val);
    hts |= val;
    if (0 != ret) {
    ...
    sc3336_write(sd, 0x320f, (unsigned char) (vts & 0xff));
    sc3336_write(sd, 0x320e, (unsigned char) (vts >> 8));
...
static int sc3336_set_hvflip(struct tx_isp_subdev *sd, void *arg)
{
...
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
static int sc3336_set_expo_short(struct tx_isp_subdev *sd, int value)
{
    int ret = ISP_SUCCESS;
...
#else
static int sc3336_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
    int ret = ISP_SUCCESS;
...
static int sc3336_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
    int ret = ISP_SUCCESS;
...
static int sc3336_set_wdr_stop(struct tx_isp_subdev *sd, void *arg)
{
    struct tx_isp_sensor *sensor = tx_isp_get_subdev_hostdata(sd);
    struct tx_isp_sensor_register_info *info = &sensor->info;
    ...
    if (par->wdr_en == 1) {
        ...
        sc3336_setting_select(sd, 1);
        sc3336_attr_set(sd, wsize);
    } else if (par->wdr_en == 0) {
        if (par->boot == -1) {
            info->default_boot = 0;
        }
        sc3336_setting_select(sd, 0);
        sc3336_attr_set(sd, wsize);
    } else {
...
static int sc3336_set_wdr(struct tx_isp_subdev *sd, void *arg)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    struct tx_isp_wdr_par *par = (struct tx_isp_wdr_par *)arg;
    struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int sc3336_pm_s_stream_ctrl(struct tx_isp_subdev *sd, void *arg)
{
	int ret = ISP_SUCCESS;
...
static int sc3336_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    case TX_ISP_EVENT_SENSOR_HVFLIP:
        if(arg)
            ret = sc3336_set_hvflip(sd, (void *)sensor_val->value);
        break;
	case TX_ISP_EVENT_PM_S_STREAM_CTRL:
		if(arg)
			ret = sc3336_pm_s_stream_ctrl(sd, arg);
		break;
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
    case TX_ISP_EVENT_SENSOR_EXPO_SHORT:
        if (arg)
            ret = sc3336_set_expo_short(sd, sensor_val->value);
        break;
#else
    case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
        if(arg)
            ret = sc3336_set_integration_time_short(sd, sensor_val->value);
        break;
    case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
        if(arg)
            ret = sc3336_set_analog_gain_short(sd, sensor_val->value);
        break;
#endif /* SENSOR_EXPO */
    case TX_ISP_EVENT_SENSOR_WDR:
        if(arg)
            ret = sc3336_set_wdr(sd, (void *)sensor_val->value);
        break;
    case TX_ISP_EVENT_SENSOR_WDR_STOP:
        if(arg)
            ret = sc3336_set_wdr_stop(sd, (void *)sensor_val->value);
        break;
#endif /* SENSOR_WDR_2_FRAME */
    default:
        break;
...
#ifdef CONFIG_ZERATUL
static int init_first_sc3336(void) {
    return private_i2c_add_driver(&sc3336_driver);
}

static void exit_first_sc3336(void) {
    private_i2c_del_driver(&sc3336_driver);
}
...
struct tx_isp_sensor_fast_attr sensor0 = {
    ...
    .wdr = 0,
#endif /* SENSOR_WDR_2_FRAME */
    .init_sensor = init_first_sc3336,
    .exit_sensor = exit_first_sc3336
};
#endif  /* CONFIG_ZERATUL */

MODULE_DESCRIPTION("A low-level driver for Sony sc3336 sensor");
MODULE_LICENSE("GPL");
