...
unsigned int gc2083_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	while (lut->gain <= gc2083_attr.again.max) {
		if (isp_gain == 0) {
			...
			*sensor_again = (lut - 1)->index;
			...
			if ((lut->gain == gc2083_attr.again.max) && (isp_gain >= lut->gain)) {
...
static int gc2083_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	ret = gc2083_write(sd, 0x0d04, it & 0xff);
	ret += gc2083_write(sd, 0x0d03, it >> 8);
...
static int gc2083_pm_s_stream_ctrl(struct tx_isp_subdev *sd, void *arg)
{
	int ret = ISP_SUCCESS;
	IMPISPUserCtl *data = (IMPISPUserCtl *)arg;

	if(data->cmd == TX_SENSOR_PM_RESUME){
		printk("gc2083 TX_SENSOR_PM_RESUME\n");
		...
		printk("gc2083 TX_SENSOR_PM_SUSPEND !!!\n");
	}else if(data->cmd == TX_SENSOR_PM_PREPARE) {
		printk("\n gc2083 TX_SENSOR_PM_PREPARE !!!\n");
...
static int gc2083_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_PM_S_STREAM_CTRL:
		if(arg)
			ret =  gc2083_pm_s_stream_ctrl(sd, arg);
		break;
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
