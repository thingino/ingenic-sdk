 ...
 *  0           2880*1620       30        mipi_2lane    linear  10        30        1.2V
 *
 * @I2C addr:0x31, 0x10
 *
 * @FSync:none
...
unsigned int gc5603_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
    ...
    while (lut->gain <= gc5603_attr.again.max) {
        if (isp_gain == 0) {
            ...
            *sensor_again = (lut - 1)->index;
            ...
            if ((lut->gain == gc5603_attr.again.max) && (isp_gain >= lut->gain)) {
...
#ifdef SENSOR_WDR_2_FRAME
unsigned int gc5603_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
#ifndef SENSOR_TEST
#ifdef SENSOR_AGAIN_TABLE
    /* Analog gain table */
    struct again_lut *lut = gc5603_again_lut;
    while(lut->gain <= gc5603_attr.max_again_short) {
        if(isp_gain == 0) {
            *sensor_again = 0;
            return 0;
        }
            ...
            *sensor_again = (lut - 1)->index;
            ...
            if((lut->gain == gc5603_attr.max_again_short) && (isp_gain >= lut->gain)) {
                *sensor_again = lut->index;
                return lut->gain;
            }
        }
...
#ifdef SENSOR_HCG
unsigned int gc5603_alloc_hcg(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
unsigned int gc5603_alloc_hcg_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
struct tx_isp_mipi_bus gc5603_mipi_linear = {
...
struct tx_isp_dvp_bus gc5603_dvp = {
...
#ifdef SENSOR_WDR_2_FRAME
    .sensor_ctrl.alloc_again_short = gc5603_alloc_again_short,
#endif /* SENSOR_WDR_2_FRAME */
...
static struct regval_list gc5603_init_regs_2880_1620_30fps_mipi[] = {
    //release_liner_v1.4_2lane_2880x1620(window)_30fps_24mhz_GC5603
    //version 1.4
    //mclk 24Mhz
...
/*
 * the order of the gc5603_win_sizes is [full_resolution, preview_resolution].
 */
static struct tx_isp_sensor_win_setting gc5603_win_sizes[] = {
    {
        ...
        .regs			= gc5603_init_regs_2880_1620_30fps_mipi,
    },
};

static struct tx_isp_sensor_win_setting *wsize = &gc5603_win_sizes[0];

/*
 * the part of driver was fixed.
 */

static struct regval_list gc5603_stream_on_mipi[] = {
    { SENSOR_REG_END, 0x00 }, /* END MARKER */
};

static struct regval_list gc5603_stream_off_mipi[] = {
    { SENSOR_REG_END, 0x00 }, /* END MARKER */
};

#ifdef SENSOR_I2C_REG_8BIT
int gc5603_read(struct tx_isp_subdev *sd, unsigned char reg,
                unsigned char *value)
{
...
int gc5603_write(struct tx_isp_subdev *sd, unsigned char reg,
                 unsigned char value)
{
    struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int gc5603_read(struct tx_isp_subdev *sd, uint16_t reg,unsigned char *value) {
    int ret;
    struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int gc5603_write(struct tx_isp_subdev *sd, uint16_t reg,unsigned char value)
{
    struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int gc5603_clk_set(struct tx_isp_subdev *sd, struct clk *sclka, int mclk)
{
...
static int gc5603_attr_set(struct tx_isp_subdev *sd, struct tx_isp_sensor_win_setting *wise)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    int ret = ISP_SUCCESS;
...
static int gc5603_setting_select(struct tx_isp_subdev *sd, int deboot)
{
    int ret = ISP_SUCCESS;
    ...
    case 0:
        ...
        gc5603_attr.again.value = 65536;
        gc5603_attr.again.max = 390142;
        gc5603_attr.again.min = 0;
        gc5603_attr.again.apply_delay = 2;

        gc5603_attr.integration_time.value = 0x127;
        gc5603_attr.integration_time.max = 1750 - 8;
        gc5603_attr.integration_time.min = 2;
        gc5603_attr.integration_time.apply_delay = 2;

        gc5603_attr.total_width =1200;
        gc5603_attr.total_height = 1750;

        gc5603_attr.expo_fs = 1;

#ifdef SENSOR_HCG
        gc5603_attr.hcg.base_gain = ;
        gc5603_attr.hcg.apply_delay = ;
#endif /* SENSOR_HCG */

#ifdef SENSOR_WDR_2_FRAME
        gc5603_attr.again_short.value = ;
        gc5603_attr.again_short.max = ;
        gc5603_attr.again_short.min = ;
        gc5603_attr.again_short.apply_delay = ;

        gc5603_attr.integration_time_short.value = ;
        gc5603_attr.integration_time_short.max = ;
        gc5603_attr.integration_time_short.min = ;
        gc5603_attr.integration_time_short.apply_delay = ;

        gc5603_attr.wdr_cache = wdr_line * gc5603_attr.total_width;

#ifdef SENSOR_HCG
        gc5603_attr.hcg_short.base_gain = ;
        gc5603_attr.hcg_short.apply_delay = ;
#endif /* SENSOR_HCG */
#endif /* SENSOR_WDR_2_FRAME */

        memcpy((void *)(&(gc5603_attr.mipi)), (void *)(&gc5603_mipi_linear), sizeof(gc5603_attr.mipi));
        break;
    default:
...
static int gc5603_attr_check(struct tx_isp_subdev *sd)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    ...
    gc5603_setting_select(sd, info->default_boot);

    switch (info->video_interface) {
    case TISP_SENSOR_VI_MIPI_CSI0:
        gc5603_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
        gc5603_attr.mipi.index = 0;
        break;
    case TISP_SENSOR_VI_MIPI_CSI1:
        gc5603_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
        gc5603_attr.mipi.index = 1;
        break;
    case TISP_SENSOR_VI_DVP:
        gc5603_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_DVP;
        break;
    default:
    ...
    ret = gc5603_clk_set(sd, sclka, SENSOR_MCLK);
    if (ret) {
                               ...
                               struct tx_isp_chip_ident *chip)
{
    ...
    gc5603_attr_check(sd);
#ifndef ZRT_SENSOR_WITHOUT_INIT
    if (info->rst_gpio != -1) {
        ret = private_gpio_request(info->rst_gpio, "gc5603_reset");
        if (!ret) {
    ...
    if (info->pwdn_gpio != -1) {
        ret = private_gpio_request(info->pwdn_gpio, "gc5603_pwdn");
        if (!ret) {
    ...
    ISP_WARNING("===================================================\n");
    ISP_WARNING("GC5603 version is %s\n", TVERSION);
...
static int gc5603_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
    ...
    if (init->enable) {
        ...
        if (sensor->video.state == TX_ISP_MODULE_INIT) {
            ret = gc5603_write_array(sd, gc5603_stream_on_mipi);
            sensor->video.state = TX_ISP_MODULE_RUNNING;
            pr_debug("gc5603 stream on\n");
        }

    } else {
        ret = gc5603_write_array(sd, gc5603_stream_off_mipi);
        sensor->video.state = TX_ISP_MODULE_INIT;
...
static int gc5603_set_expo(struct tx_isp_subdev *sd, int value)
{
    ...
    ret = gc5603_write(sd, 0x0203, it & 0xff);
    ret += gc5603_write(sd, 0x0202, it >> 8);

    /*set sensor analog gain*/
    ret += gc5603_write(sd, 0x031d ,0x2d);
    ret += gc5603_write(sd, 0x0614, val_lut[again].reg0614);
    ret += gc5603_write(sd, 0x0615, val_lut[again].reg0615);
    ret += gc5603_write(sd, 0x0225, val_lut[again].reg0225);
    ...
    ret += gc5603_write(sd, 0x00b8, val_lut[again].reg00b8);
    ret += gc5603_write(sd, 0x00b9, val_lut[again].reg00b9);
...
static int gc5603_set_fps(struct tx_isp_subdev *sd, int fps)
{
    ...
    val = 0;
    ret += gc5603_read(sd, 0x0342, &val);
    hts = (val & 0x0f) << 8;
    val = 0;
    ret += gc5603_read(sd, 0x0343, &val);
    hts  = (hts | val);
    if (0 != ret) {
        ISP_ERROR("err: gc5603 read err\n");
        return ret;
    }

    vts = sclk * (fps & 0xffff) / hts / ((fps & 0xffff0000) >> 16);

    gc5603_write(sd, 0x0340, (unsigned char) ((vts >> 8) & 0x3f));
    gc5603_write(sd, 0x0341, (unsigned char) (vts & 0xff));

    gc5603_write(sd, 0x0259, (unsigned char)(((vts - 32) >> 8 ) & 0x3f));
    gc5603_write(sd, 0x025a, (unsigned char)((vts - 32) & 0xff));

    if (0 != ret) {
        ISP_ERROR("err: gc5603_write err\n");
        return ret;
    }
...
static int gc5603_set_hvflip(struct tx_isp_subdev *sd, void *arg)
{
    ...
    ret = gc5603_read(sd, 0x022c, &val);
    ret = gc5603_read(sd, 0x0063, &val1);
    ret = gc5603_read(sd, 0x0a73, &val2);
    ret = gc5603_read(sd, 0x0348, &reg_0x0348);
    ret = gc5603_read(sd, 0x0349, &reg_0x0349);
    ret = gc5603_read(sd, 0x034a, &reg_0x034a);
    ret = gc5603_read(sd, 0x034b, &reg_0x034b);
    ...
    case TX_ISP_SENSOR_HVFLIP_NOMAL:
        ret = gc5603_write(sd, 0x022c, val	| 0x00); /*mirror*/
        ret = gc5603_write(sd, 0x0063, val1 | 0x00);
        ret = gc5603_write(sd, 0x0a73, val2 | 0x60);
        break;
    case TX_ISP_SENSOR_HVFLIP_HFLIP:
        ret = gc5603_write(sd, 0x022c, val	| 0x01); /*mirror*/
        ret = gc5603_write(sd, 0x0063, val1 | 0x01);
        ret = gc5603_write(sd, 0x0a73, val2 | 0x61);
        break;
    case TX_ISP_SENSOR_HVFLIP_VFLIP:
        ret = gc5603_write(sd, 0x022c, val	| 0x02); /*filp*/
        ret = gc5603_write(sd, 0x0063, val1 | 0x02);
        ret = gc5603_write(sd, 0x0a73, val2 | 0x62);
        break;
    case TX_ISP_SENSOR_HVFLIP_HVFLIP:
        ret = gc5603_write(sd, 0x022c, val	| 0x03); /*mirror & filp*/
        ret = gc5603_write(sd, 0x0063, val1 | 0x03);
        ret = gc5603_write(sd, 0x0a73, val2 | 0x63);
        break;
    }

    ret = gc5603_write(sd, 0x031f,0x01);
    ret = gc5603_write(sd, 0x031f,0x00);
    ret = gc5603_write(sd, 0x0317,0x1c);
    ret = gc5603_write(sd, 0x0a67,0x80);
    ret = gc5603_write(sd, 0x0a50,0x80);
    ret = gc5603_write(sd, 0x0a53,0x00);
    ret = gc5603_write(sd, 0x0a51,0x80);
    ret = gc5603_write(sd, 0x0a54,0xa0);
    ret = gc5603_write(sd, 0x0a63,0x20);
    ret = gc5603_write(sd, 0x0a65,0x20);
    ret = gc5603_write(sd, 0x0a68,0x21);
    ret = gc5603_write(sd, 0x05be,0x00);
    ret = gc5603_write(sd, 0x05a9,0x01);
    ret = gc5603_write(sd, 0x0024,0x00);
    ret = gc5603_write(sd, 0x0025,0x00);
    ret = gc5603_write(sd, 0x0028,reg_0x0348);
    ret = gc5603_write(sd, 0x0029,reg_0x0349);
    ret = gc5603_write(sd, 0x002a,reg_0x034a);
    ret = gc5603_write(sd, 0x002b,reg_0x034b);
    ret = gc5603_write(sd, 0x0a82,0x00);
    ret = gc5603_write(sd, 0x0a83,0xe0);
    ret = gc5603_write(sd, 0x0a72,0x02);
    ret = gc5603_write(sd, 0x0a73,val2);
    ret = gc5603_write(sd, 0x0a80,0x01);
    ret = gc5603_write(sd, 0x0a70,0x03);
    ret = gc5603_write(sd, 0x0a75,0x41);
    ret = gc5603_write(sd, 0x0a5a,0x80);

    private_msleep(0x14);    
    ret = gc5603_write(sd, 0x05be,0x01);
    ret = gc5603_write(sd, 0x0a70,0x00);
    ret = gc5603_write(sd, 0x0080,0x02);
    ret = gc5603_write(sd, 0x0021,0x40);
    ret = gc5603_write(sd, 0x0a67,0x00);
    ret = gc5603_write(sd, 0x0100,0x09);
...
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
static int gc5603_set_expo_short(struct tx_isp_subdev *sd, int value)
{
    int ret = ISP_SUCCESS;
...
#else
static int gc5603_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
    int ret = ISP_SUCCESS;
...
static int gc5603_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
    int ret = ISP_SUCCESS;
...
static int gc5603_set_wdr_stop(struct tx_isp_subdev *sd, void *arg)
{
    ...
    if (par->wdr_en == 1) {
        ...
        gc5603_setting_select(sd, 1);
        gc5603_attr_set(sd, wsize);
    } else if (par->wdr_en == 0) {
        if (par->boot == -1) {
            info->default_boot = 0;
        }
        gc5603_setting_select(sd, 0);
        gc5603_attr_set(sd, wsize);
    } else {
...
static int gc5603_set_wdr(struct tx_isp_subdev *sd, void *arg)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    struct tx_isp_wdr_par *par = (struct tx_isp_wdr_par *)arg;
    struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int gc5603_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    case TX_ISP_EVENT_SENSOR_HVFLIP:
        if(arg)
            ret = gc5603_set_hvflip(sd, (void *)sensor_val->value);
        break;
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
    case TX_ISP_EVENT_SENSOR_EXPO_SHORT:
        if (arg)
            ret = gc5603_set_expo_short(sd, sensor_val->value);
        break;
#else
    case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
        if(arg)
            ret = gc5603_set_integration_time_short(sd, sensor_val->value);
        break;
    case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
        if(arg)
            ret = gc5603_set_analog_gain_short(sd, sensor_val->value);
        break;
#endif /* SENSOR_EXPO */
    case TX_ISP_EVENT_SENSOR_WDR:
        if(arg)
            ret = gc5603_set_wdr(sd, (void *)sensor_val->value);
        break;
    case TX_ISP_EVENT_SENSOR_WDR_STOP:
        if(arg)
            ret = gc5603_set_wdr_stop(sd, (void *)sensor_val->value);
        break;
#endif /* SENSOR_WDR_2_FRAME */
    default:
        break;
...
#ifdef CONFIG_ZERATUL
static int init_first_gc5603(void) {
    return private_i2c_add_driver(&gc5603_driver);
}

static void exit_first_gc5603(void) {
    private_i2c_del_driver(&gc5603_driver);
}
...
struct tx_isp_sensor_fast_attr sensor0 = {
    ...
    .wdr = 0,
#endif /* SENSOR_WDR_2_FRAME */
    .init_sensor = init_first_gc5603,
    .exit_sensor = exit_first_gc5603
};
#endif	/* CONFIG_ZERATUL */

MODULE_DESCRIPTION("A low-level driver for gc5603 sensor");
MODULE_LICENSE("GPL");
