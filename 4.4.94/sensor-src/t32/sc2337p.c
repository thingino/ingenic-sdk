 ...
 *   0		1920x1080	25	    MIPI       Linear   RAW10     25
 *
 * @I2C addr:
 *
 * @FSync:
...
unsigned int sc2337p_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	while (lut->gain <= sc2337p_attr.again.max) {
		if (isp_gain == 0) {
			...
			*sensor_again = (lut - 1)->value;
			...
			if ((lut->gain == sc2337p_attr.again.max) && (isp_gain >= lut->gain)) {
...
#ifdef SENSOR_WDR_2_FRAME
unsigned int sc2337p_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
#ifndef SENSOR_TEST
#ifdef SENSOR_AGAIN_TABLE
	/* Analog gain table */
	struct again_lut *lut = sc2337p_again_lut;
	while(lut->gain <= sc2337p_attr.again_short.max) {
		if(isp_gain == 0) {
			*sensor_again = 0;
			return 0;
		}
			...
			*sensor_again = (lut - 1)->value;
			...
			if((lut->gain == sc2337p_attr.again_short.max) && (isp_gain >= lut->gain)) {
				*sensor_again = lut->value;
				return lut->gain;
			}
		}
...
#ifdef SENSOR_HCG
unsigned int sc2337p_alloc_hcg(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
unsigned int sc2337p_alloc_hcg_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
struct tx_isp_mipi_bus sc2337p_mipi_linear = {
...
struct tx_isp_dvp_bus sc2337p_dvp = {
...
#ifdef SENSOR_WDR_2_FRAME
	.sensor_ctrl.alloc_again_short = sc2337p_alloc_again_short,
#endif /* SENSOR_WDR_2_FRAME */
...
static struct regval_list sc2337p_init_regs_1920_1080_25fps_mipi[] = {
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
...
/*
 * the order of the sc2337p_win_sizes is [full_resolution, preview_resolution].
 */
static struct tx_isp_sensor_win_setting sc2337p_win_sizes[] = {
	/* 1920*1080 [0] */
	{
		...
		.regs           = sc2337p_init_regs_1920_1080_25fps_mipi,
	},
};

static struct tx_isp_sensor_win_setting *wsize = &sc2337p_win_sizes[0];
...
#ifdef SENSOR_I2C_REG_8BIT
int sc2337p_read(struct tx_isp_subdev *sd, unsigned char reg,
		  unsigned char *value)
{
...
int sc2337p_write(struct tx_isp_subdev *sd, unsigned char reg,
		   unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int sc2337p_read(struct tx_isp_subdev *sd, uint16_t reg,
		  unsigned char *value) {
	int ret;
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int sc2337p_write(struct tx_isp_subdev *sd, uint16_t reg,
		   unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int sc2337p_clk_set(struct tx_isp_subdev *sd, struct clk *sclka, int mclk)
{
...
static int sc2337p_attr_set(struct tx_isp_subdev *sd, struct tx_isp_sensor_win_setting *wise)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
...
static int sc2337p_setting_select(struct tx_isp_subdev *sd, int deboot)
{
	int ret = ISP_SUCCESS;
	...
	case 0:
		...
		sc2337p_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;

		sc2337p_attr.again.value = 0;
		sc2337p_attr.again.max = 391733;
		sc2337p_attr.again.min = 0;
		sc2337p_attr.again.apply_delay = 2;

		sc2337p_attr.integration_time.value = 0x59a;
		sc2337p_attr.integration_time.max = 0x5a0 - 6;
		sc2337p_attr.integration_time.min = 1;
		sc2337p_attr.integration_time.apply_delay = 2;

		sc2337p_attr.total_width = 2200;
		sc2337p_attr.total_height = 0x5a0;

		sc2337p_attr.expo_fs = 1;

#ifdef SENSOR_HCG
		sc2337p_attr.hcg.base_gain = ;
		sc2337p_attr.hcg.apply_delay = ;
#endif /* SENSOR_HCG */

#ifdef SENSOR_WDR_2_FRAME
		sc2337p_attr.again_short.value = ;
		sc2337p_attr.again_short.max = ;
		sc2337p_attr.again_short.min = ;
		sc2337p_attr.again_short.apply_delay = ;

		sc2337p_attr.integration_time_short.value = ;
		sc2337p_attr.integration_time_short.max = ;
		sc2337p_attr.integration_time_short.min = ;
		sc2337p_attr.integration_time_short.apply_delay = ;

		sc2337p_attr.wdr_cache = wdr_line * sc2337p_attr.total_width;

#ifdef SENSOR_HCG
		sc2337p_attr.hcg_short.base_gain = ;
		sc2337p_attr.hcg_short.apply_delay = ;
#endif /* SENSOR_HCG */
#endif /* SENSOR_WDR_2_FRAME */

		memcpy((void *)(&(sc2337p_attr.mipi)), (void *)(&sc2337p_mipi_linear), sizeof(sc2337p_attr.mipi));
		break;
	default:
...
static int sc2337p_attr_check(struct tx_isp_subdev *sd)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	...
	sc2337p_setting_select(sd, info->default_boot);

	switch (info->video_interface) {
	case TISP_SENSOR_VI_MIPI_CSI0:
		sc2337p_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		sc2337p_attr.mipi.index = 0;
		break;
	case TISP_SENSOR_VI_MIPI_CSI1:
		sc2337p_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		sc2337p_attr.mipi.index = 1;
		break;
	case TISP_SENSOR_VI_DVP:
		sc2337p_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_DVP;
		break;
	default:
	...
	ret = sc2337p_clk_set(sd, sclka, SENSOR_MCLK);
	if (ret) {
				 ...
				 struct tx_isp_chip_ident *chip)
{
	...
	sc2337p_attr_check(sd);
#ifndef ZRT_SENSOR_WITHOUT_INIT
	if (info->rst_gpio != -1) {
		ret = private_gpio_request(info->rst_gpio, "sc2337p_reset");
		if (!ret) {
	...
	if (info->pwdn_gpio != -1) {
		ret = private_gpio_request(info->pwdn_gpio, "sc2337p_pwdn");
		if (!ret) {
	...
	ISP_WARNING("===================================================\n");
	ISP_WARNING("sc2337p version is %s\n", TVERSION);
...
static int sc2337p_reset(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	return 0;
}

static int sc2337p_init(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
...
static int sc2337p_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			...
			pr_debug("sc2337p stream on\n");
		}

	} else {
		ret = sc2337p_write_array(sd, sc2337p_stream_off_mipi);
		sensor->video.state = TX_ISP_MODULE_INIT;
		pr_debug("sc2337p stream off\n");
	}
...
static int sc2337p_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	case 0:
		sclk = 0x5a0 * 0x898 * 25;  /**< HTS * VTS * FPS */
		max_fps = wsize->fps;
		break;
	default:
		ret = -1;
	...
	val = 0;
	ret += sc2337p_read(sd, 0x320c, &val);
	hts = val << 8;
	val = 0;
	ret += sc2337p_read(sd, 0x320d, &val);
	hts |= val;
	if (0 != ret) {
	...
	sc2337p_write(sd, 0x320f, (unsigned char) (vts & 0xff));
	sc2337p_write(sd, 0x320e, (unsigned char) (vts >> 8));
...
static int sc2337p_set_hvflip(struct tx_isp_subdev *sd, void *arg)
{
...
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
static int sc2337p_set_expo_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
#else
static int sc2337p_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
static int sc2337p_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
static int sc2337p_set_wdr_stop(struct tx_isp_subdev *sd, void *arg)
{
	struct tx_isp_sensor *sensor = tx_isp_get_subdev_hostdata(sd);
	struct tx_isp_sensor_register_info *info = &sensor->info;
	...
	if (par->wdr_en == 1) {
		...
		sc2337p_setting_select(sd, 1);
		sc2337p_attr_set(sd, wsize);
	} else if (par->wdr_en == 0) {
		if (par->boot == -1) {
			info->default_boot = 0;
		}
		sc2337p_setting_select(sd, 0);
		sc2337p_attr_set(sd, wsize);
	} else {
...
static int sc2337p_set_wdr(struct tx_isp_subdev *sd, void *arg)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	struct tx_isp_wdr_par *par = (struct tx_isp_wdr_par *)arg;
	struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int sc2337p_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if (arg)
			ret = sc2337p_set_expo(sd, sensor_val->value);
		break;
#else
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if (arg)
			ret = sc2337p_set_integration_time(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if (arg)
			ret = sc2337p_set_analog_gain(sd, sensor_val->value);
		break;
#endif /* SENSOR_EXPO */
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if (arg)
			ret = sc2337p_set_digital_gain(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if (arg)
			ret = sc2337p_get_black_pedestal(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if (arg)
			ret = sc2337p_set_mode(sd, sensor_val->value);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if (arg)
			ret = sc2337p_set_fps(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_HVFLIP:
		if(arg)
			ret = sc2337p_set_hvflip(sd, (void *)sensor_val->value);
		break;
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
	case TX_ISP_EVENT_SENSOR_EXPO_SHORT:
		if (arg)
			ret = sc2337p_set_expo_short(sd, sensor_val->value);
		break;
#else
	case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
		if(arg)
			ret = sc2337p_set_integration_time_short(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
		if(arg)
			ret = sc2337p_set_analog_gain_short(sd, sensor_val->value);
		break;
#endif /* SENSOR_EXPO */
	case TX_ISP_EVENT_SENSOR_WDR:
		if(arg)
			ret = sc2337p_set_wdr(sd, (void *)sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_WDR_STOP:
		if(arg)
			ret = sc2337p_set_wdr_stop(sd, (void *)sensor_val->value);
		break;
#endif /* SENSOR_WDR_2_FRAME */
	default:
		break;
...
static int sc2337p_probe(struct i2c_client *client,
...
#ifdef CONFIG_ZERATUL
static int init_first_sc2337p(void) {
	return private_i2c_add_driver(&sc2337p_driver);
}

static void exit_first_sc2337p(void) {
	private_i2c_del_driver(&sc2337p_driver);
}
...
struct tx_isp_sensor_fast_attr sensor0 = {
	...
	.wdr = 0,
#endif /* SENSOR_WDR_2_FRAME */
	.init_sensor = init_first_sc2337p,
	.exit_sensor = exit_first_sc2337p
};
#endif  /* CONFIG_ZERATUL */

MODULE_DESCRIPTION("A low-level driver for sc2337p sensor");
MODULE_LICENSE("GPL");
