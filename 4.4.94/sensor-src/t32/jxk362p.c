 ...
 *   0          3200x1800       25          MIPI        Linear   10      25
 * @I2C addr:
 *
 * @FSync:
 *
...
unsigned int jxk362p_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	while (lut->gain <= jxk362p_attr.again.max) {
		if (isp_gain == 0) {
			...
			*sensor_again = (lut - 1)->value;
			...
			if ((lut->gain == jxk362p_attr.again.max) && (isp_gain >= lut->gain)) {
...
#ifdef SENSOR_WDR_2_FRAME
unsigned int jxk362p_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
#ifndef SENSOR_TEST
#ifdef SENSOR_AGAIN_TABLE
	/* Analog gain table */
	struct again_lut *lut = jxk362p_again_lut;
	while(lut->gain <= jxk362p_attr.again_short.max) {
		if(isp_gain == 0) {
			*sensor_again = 0;
			return 0;
		}
			...
			*sensor_again = (lut - 1)->value;
			...
			if((lut->gain == jxk362p_attr.again_short.max) && (isp_gain >= lut->gain)) {
				*sensor_again = lut->value;
				return lut->gain;
			}
		}
...
#ifdef SENSOR_HCG
unsigned int jxk362p_alloc_hcg(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
unsigned int jxk362p_alloc_hcg_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
struct tx_isp_dvp_bus jxk362p_dvp = {
...
#ifdef SENSOR_WDR_2_FRAME
	.sensor_ctrl.alloc_again_short = jxk362p_alloc_again_short,
#endif /* SENSOR_WDR_2_FRAME */
...
static struct regval_list jxk362p_init_regs_3200_1800_25fps_mipi[] = {
	//[K362P(AC)_006_20250326_MIPI10_2L_3200x1800x25_M24_P172P8_F3600x1920_H4_V1]
	//Width=3200
	//Height=1800
	//FrameWidth=3600
	//FrameHeight=1920
...
static struct tx_isp_sensor_win_setting jxk362p_win_sizes[] = {
	/* 2560*1440 [0] */
	{
		...
		.regs           = jxk362p_init_regs_3200_1800_25fps_mipi,
	},
};

static struct tx_isp_sensor_win_setting *wsize = &jxk362p_win_sizes[0];
...
#if 0
static int jxk362p_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
...
int jxk362p_read(struct tx_isp_subdev *sd, uint16_t reg,
		  unsigned char *value) {
	int ret;
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int jxk362p_write(struct tx_isp_subdev *sd, uint16_t reg,
		   unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int jxk362p_clk_set(struct tx_isp_subdev *sd, struct clk *sclka, int mclk)
{
...
static int jxk362p_attr_set(struct tx_isp_subdev *sd, struct tx_isp_sensor_win_setting *wise)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
...
static int jxk362p_setting_select(struct tx_isp_subdev *sd, int deboot)
{
	int ret = ISP_SUCCESS;
	...
	case 0:
		...
		jxk362p_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;

		jxk362p_attr.again.value = 0x64;
		jxk362p_attr.again.max = 324678;
		jxk362p_attr.again.min = 0;
		jxk362p_attr.again.apply_delay = 2;

		jxk362p_attr.integration_time.value = 1920 - 4 ;
		jxk362p_attr.integration_time.max = 1920 - 4;
		jxk362p_attr.integration_time.min = 1;
		jxk362p_attr.integration_time.apply_delay = 2;

		jxk362p_attr.total_width = 3600;
		jxk362p_attr.total_height = 1920;

		jxk362p_attr.expo_fs = 1;

#ifdef SENSOR_HCG
		jxk362p_attr.hcg.base_gain = ;
		jxk362p_attr.hcg.apply_delay = ;
#endif /* SENSOR_HCG */

#ifdef SENSOR_WDR_2_FRAME
		jxk362p_attr.again_short.value = ;
		jxk362p_attr.again_short.max = ;
		jxk362p_attr.again_short.min = ;
		jxk362p_attr.again_short.apply_delay = ;

		jxk362p_attr.integration_time_short.value = ;
		jxk362p_attr.integration_time_short.max = ;
		jxk362p_attr.integration_time_short.min = ;
		jxk362p_attr.integration_time_short.apply_delay = ;

		jxk362p_attr.wdr_cache = wdr_line * jxk362p_attr.total_width;

#ifdef SENSOR_HCG
		jxk362p_attr.hcg_short.base_gain = ;
		jxk362p_attr.hcg_short.apply_delay = ;
#endif /* SENSOR_HCG */
#endif /* SENSOR_WDR_2_FRAME */

		memcpy((void *)(&(jxk362p_attr.mipi)), (void *)(&jxk362p_mipi_linear), sizeof(jxk362p_attr.mipi));
		break;
	default:
...
static int jxk362p_attr_check(struct tx_isp_subdev *sd)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	...
	jxk362p_setting_select(sd, info->default_boot);
	...
	ret = jxk362p_clk_set(sd, sclka, SENSOR_MCLK);
	if (ret) {
				 ...
				 struct tx_isp_chip_ident *chip)
{
	...
	ISP_WARNING("===================================================\n");
	ISP_WARNING("jxk362p version is %s\n", TVERSION);
...
static int jxk362p_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			...
			pr_debug("jxk362p stream on\n");
		}

	} else {
		ret = jxk362p_write_array(sd, jxk362p_stream_off_mipi);
		sensor->video.state = TX_ISP_MODULE_INIT;
		pr_debug("jxk362p stream off\n");
	}
...
static int jxk362p_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	ret = jxk362p_write(sd,  0x01, (unsigned char)(it & 0xff));
	ret += jxk362p_write(sd, 0x02, (unsigned char)((it >> 8) & 0xff));
	ret += jxk362p_write(sd, 0x00, (unsigned char)(again & 0x7f));
...
static int jxk362p_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	case 0:
		sclk = 900 * 1920 * 25 * 4;  /**< HTS * VTS * FPS */
		max_fps = 25;
		break;
	default:
		ret = -1;
	...
	jxk362p_write(sd, 0x22, (unsigned char) (vts & 0xff));
	jxk362p_write(sd, 0x23, (unsigned char) (vts >> 8));
...
static int jxk362p_set_hvflip(struct tx_isp_subdev *sd, void *arg)
{
...
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
static int jxk362p_set_expo_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
#else
static int jxk362p_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
static int jxk362p_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
static int jxk362p_set_wdr_stop(struct tx_isp_subdev *sd, void *arg)
{
	struct tx_isp_sensor *sensor = tx_isp_get_subdev_hostdata(sd);
	struct tx_isp_sensor_register_info *info = &sensor->info;
	...
	if (par->wdr_en == 1) {
		...
		jxk362p_setting_select(sd, 1);
		jxk362p_attr_set(sd, wsize);
	} else if (par->wdr_en == 0) {
		if (par->boot == -1) {
			info->default_boot = 0;
		}
		jxk362p_setting_select(sd, 0);
		jxk362p_attr_set(sd, wsize);
	} else {
...
static int jxk362p_set_wdr(struct tx_isp_subdev *sd, void *arg)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	struct tx_isp_wdr_par *par = (struct tx_isp_wdr_par *)arg;
	struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int jxk362p_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_HVFLIP:
		if(arg)
			ret = jxk362p_set_hvflip(sd, (void *)sensor_val->value);
		break;
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
	case TX_ISP_EVENT_SENSOR_EXPO_SHORT:
		if (arg)
			ret = jxk362p_set_expo_short(sd, sensor_val->value);
		break;
#else
	case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
		if(arg)
			ret = jxk362p_set_integration_time_short(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
		if(arg)
			ret = jxk362p_set_analog_gain_short(sd, sensor_val->value);
		break;
#endif /* SENSOR_EXPO */
	case TX_ISP_EVENT_SENSOR_WDR:
		if(arg)
			ret = jxk362p_set_wdr(sd, (void *)sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_WDR_STOP:
		if(arg)
			ret = jxk362p_set_wdr_stop(sd, (void *)sensor_val->value);
		break;
#endif /* SENSOR_WDR_2_FRAME */
	default:
		break;
...
static int jxk362p_probe(struct i2c_client *client,
...
#ifdef CONFIG_ZERATUL
static int init_first_jxk362p(void) {
	return private_i2c_add_driver(&jxk362p_driver);
}

static void exit_first_jxk362p(void) {
	private_i2c_del_driver(&jxk362p_driver);
}
...
struct tx_isp_sensor_fast_attr sensor0 = {
	...
	.wdr = 0,
#endif /* SENSOR_WDR_2_FRAME */
	.init_sensor = init_first_jxk362p,
	.exit_sensor = exit_first_jxk362p
};
#endif  /* CONFIG_ZERATUL */

MODULE_DESCRIPTION("A low-level driver for jxk362p sensor");
MODULE_LICENSE("GPL");
