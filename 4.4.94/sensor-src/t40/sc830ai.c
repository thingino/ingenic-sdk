...
#define SENSOR_SUPPORT_SCLK_8M_FPS_30_2LANE (141750000)
#define SENSOR_SUPPORT_SCLK_8M_FPS_20_2LANE (189000000)
...
/*
 * 3840x2160@15 over 2 MIPI data lanes: boot 2 of the stock eufy T8416
 * sensor_sc830ai_t40.ko, copied byte-for-byte. That camera runs this mode
 * with both CSI ports at 2 lanes alongside an sc3338.
 */
static struct regval_list sensor_init_regs_3840_2160_15fps_2lane_mipi[] = {
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
	{0x3018,0x32}, //2 lanes (4-lane tables leave the 0x72 default)
	{0x3019,0x0c}, //lane mask
	{0x301f,0x07},
	{0x320c,0x09}, //hts 0x960=2400; vts not written (2250)
	{0x320d,0x60},
	{0x3221,0x00}, //flip[6:5] mirror[2:1]
	{0x3281,0x80},
	{0x3301,0x0e},
	{0x3303,0x18},
	{0x3306,0x50},
...
/* T41 driver H202300505a, boot 0 */
static struct regval_list sensor_init_regs_3840_2160_30fps_2lane_mipi[] = {
	{0x0103, 0x01},
	{0x0100, 0x00},
	{0x36e9, 0x80},
	{0x37f9, 0x80},
...
/* T41ZRT driver H20221031a */
static struct regval_list sensor_init_regs_3840_2160_20fps_2lane_mipi[] = {
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
...
static struct tx_isp_sensor_win_setting sensor_win_sizes[] = {
	...
	/* 3840*2160, 2 lanes */
	{
		...
		.mbus_code = TISP_VI_FMT_SBGGR10_1X10,
		.colorspace = TISP_COLORSPACE_SRGB,
		.regs = sensor_init_regs_3840_2160_15fps_2lane_mipi,
	},
	/* 3840*2160, 2 lanes */
	{
		.width = 3840,
		...
		.mbus_code = TISP_VI_FMT_SBGGR10_1X10,
		.colorspace = TISP_COLORSPACE_SRGB,
		.regs = sensor_init_regs_3840_2160_30fps_2lane_mipi,
	},
	/* 3840*2160, 2 lanes */
	{
		.width = 3840,
		...
		.mbus_code = TISP_VI_FMT_SBGGR10_1X10,
		.colorspace = TISP_COLORSPACE_SRGB,
		.regs = sensor_init_regs_3840_2160_20fps_2lane_mipi,
	}
};
...
static int sensor_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	case 1:
		...
		break;
	case 2:
		sclk = SENSOR_SUPPORT_SCLK_8M_FPS_15_2LANE;
		max_fps = TX_SENSOR_MAX_FPS_15;
		break;
	case 3:
		sclk = SENSOR_SUPPORT_SCLK_8M_FPS_30_2LANE;
		max_fps = TX_SENSOR_MAX_FPS_30;
		break;
	case 4:
		sclk = SENSOR_SUPPORT_SCLK_8M_FPS_20_2LANE;
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	struct clk *sclka;
        ...
        case 0:
	        ...
	        sensor_attr.again = 0;
                sensor_attr.integration_time = 0x11c6;
                break;
        case 1:
        ...
        case 3:
                ...
                sensor_attr.integration_time_limit = 2241;
                sensor_attr.max_integration_time = 2241;
                sensor_attr.total_width = 0x834 * 2;
                sensor_attr.total_height = 2250;
                break;
        case 4:
                ...
                sensor_attr.integration_time_limit = 2241;
                sensor_attr.max_integration_time = 2241;
                sensor_attr.total_width = 0x1068 * 2;
                sensor_attr.total_height = 2250;
                break;
        default:
	...
	if (info->default_boot == 3) {
		/* boot 3 is the T41 table, timed for a 27 MHz MCLK; apll has no
		 * 27 MHz divisor, so run this mux from vpll at 891 MHz */
		if (((rate / 1000) % 27000) != 0) {
			struct clk *vpll = clk_get(NULL, "vpll");

			if (IS_ERR(sclka) || IS_ERR(vpll) || clk_set_parent(sclka, vpll))
				ISP_ERROR("Cannot move MCLK onto vpll\n");
			else if (((private_clk_get_rate(vpll) / 1000) % 27000) != 0)
				private_clk_set_rate(vpll, 891000000);
		}
